...
const Documents = () => {
  ...
  const filteredItems = useMemo(() => {
    ...
    return list
      .filter((item) => item && item.name !== ".placeholder")
    ...
    <div className="min-h-screen bg-[#FDFDFD] dark:bg-slate-950 transition-colors duration-300 relative font-sans">
      ...
      <main className="max-w-450 mx-auto px-4 sm:px-10 py-8 space-y-6 pb-40">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          <div>
            <h1 className="text-2xl font-[1000] text-slate-950 dark:text-white uppercase tracking-tight">
              Documents
            </h1>
            ...
            <div className="flex items-center gap-1 mt-1">
              {["Main Folder", ...currentPath].map((name, i) => {
                ...
                const displayName =
                  isFamilyBranch && nameMap[name] ? nameMap[name] : name;
        ...
        {uploading && (
          <div className="bg-emerald-50 border border-emerald-100 dark:border-emerald-900/10 rounded-2xl p-4 animate-in slide-in-from-top-2">
            <div className="flex justify-between mb-2 text-[11px] font-black uppercase text-emerald-700">
              <span>
                Uploading {activeUploadName} ({uploadQueue.current}/
                {uploadQueue.total})
              </span>
        ...
        <div
          className={
            viewMode === "grid"
              ...
              : "flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm"
          }
        >
          {viewMode === "list" && (
          ...
          {initialLoading ? (
            Array(8)
              ...
              .map((_, i) => (
                <div
                  key={i}
                  className={`bg-slate-100 dark:bg-slate-800/40 rounded-2xl animate-pulse ${viewMode === "grid" ? "aspect-square" : "h-14 mb-1 mx-2"}`}
                />
              ))
          ) : (
            <>
              {!searchTerm && (
                <label
                  className={
                    viewMode === "grid"
                      ? "group relative bg-slate-50 dark:bg-slate-700/50 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl p-5 flex flex-col items-center justify-center cursor-pointer hover:border-emerald-500 transition-all aspect-square"
                      : "flex items-center gap-4 px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer border-b border-slate-100 dark:border-slate-800 transition-all"
                  }
                >
                  <Upload
                    size={viewMode === "grid" ? 24 : 18}
                  ...
                  <input
                    ...
                    className="hidden"
                    onChange={(e) =>
                      startBatchUpload(Array.from(e.target.files))
                    }
                  />
              ...
              {isCreatingFolder && !searchTerm && (
                <div
                  className={
                    viewMode === "grid"
                      ? "bg-white dark:bg-slate-700 border-2 border-emerald-500 rounded-2xl p-4 flex flex-col items-center justify-center animate-in zoom-in-95 aspect-square"
                      : "flex items-center gap-4 px-6 py-4 border-b border-emerald-500 bg-emerald-50/10"
                  }
                >
                  <Folder
                    size={viewMode === "grid" ? 32 : 18}
                  ...
                  <input
                    ...
                    onKeyDown={async (e) => {
                      if (e.key === "Enter" && editValue.trim()) {
                        await handleCreateFolder(editValue.trim());
                        setIsCreatingFolder(false);
                        setEditValue("");
                      }
                    }}
                    className="bg-transparent text-[10px] font-black outline-none border-b border-emerald-500 uppercase ml-2 flex-1"
                    placeholder="NAME..."
                  />
                </div>
              )}
              {filteredItems.map((item) => (
                <FileCard
                  ...
                  onGoToFolder={() =>
                    setCurrentPath(
                      item.parentPath ? item.parentPath.split("/") : [],
                    )
                  }
                />
              ))}
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex justify-between text-[9px] font-black uppercase text-slate-500">
              <span>Used: {totalSizeMB.toFixed(1)} MB</span>
            </div>
            <div className="h-1 bg-slate-100 dark:bg-slate-900 rounded-full mt-1 overflow-hidden border border-slate-200 dark:border-slate-800">
              <div
                className="h-full bg-emerald-600 transition-all shadow-[0_0_8px_rgba(16,185,129,0.5)]"
        ...
        {selectedIds.length > 0 && (
          <div className="flex-1 flex justify-end gap-3 items-center animate-in slide-in-from-bottom-2">
            ...
            <button
              ...
              className="px-4 py-2 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase shadow-lg"
            >
              Download
            </button>
            <button
              ...
              className="px-4 py-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl text-[10px] font-black uppercase"
            >
              Move
            </button>
            <button
            ...
            <button
              ...
              className="p-2 text-slate-400 hover:text-slate-950 dark:hover:text-white transition-colors"
            >
              <X size={20} />
            </button>
          </div>
