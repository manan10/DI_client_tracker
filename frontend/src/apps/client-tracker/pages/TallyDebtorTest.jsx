...
const TallyDebtorTest = () => {
  ...
  const [selectedLedgerForInspect, setSelectedLedgerForInspect] =
    useState(null);
  ...
  const fetchCompanies = async () => {
    ...
    try {
      ...
      logActivity(
        `Bridge Failed. Verify Tally is open with XML port 9000 enabled.`,
        "error",
      );
    }
  };
  ...
  const fetchSundryDebtors = async () => {
    ...
    logActivity(
      `Executing getLedgersTest [mode: ${fetchMode}] for "${selectedCompany}"...`,
      "process",
    );
    ...
    try {
      ...
      logActivity(
        `Raw response parsed: ${ledgerMatches.length} total ledgers returned.`,
        "info",
      );
      ...
      ledgerMatches.forEach((m) => {
        ...
        const isUnderDebtors =
          parent.toUpperCase().includes("SUNDRY DEBTORS") ||
        ...
        const stateMatch =
          block.match(/<LEDSTATENAME[^>]*>(.*?)<\/LEDSTATENAME>/i) ||
        ...
        const panMatch =
          block.match(/<INCOMETAXNUMBER[^>]*>(.*?)<\/INCOMETAXNUMBER>/i) ||
          block.match(/<PANNUMBER[^>]*>(.*?)<\/PANNUMBER>/i);
        const pan = panMatch
          ? unescapeXml(panMatch[1]).trim()
          : gstin
        ...
        const inferredGstPrefix =
          stateCode && pan && pan.length === 10 ? `${stateCode}${pan}` : "";
        ...
        const gstRegType = regTypeMatch
          ? unescapeXml(regTypeMatch[1]).trim()
          : gstin
            ? "Regular"
            : "Unregistered";
        ...
        // 9. Bill-wise
        const billWiseMatch = block.match(
          /<ISBILLWISEON[^>]*>(.*?)<\/ISBILLWISEON>/i,
        );
      ...
      logActivity(
        `Filtered ${parsedList.length} Mutual Fund accounts under Sundry Debtors.`,
        "success",
      );
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0B1120] text-slate-800 dark:text-slate-100 font-sans">
      ...
      <main className="w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl p-5 shadow-xs">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">
                  Inspecting root tags and sub-collections for missing AMC
                  GSTINs
                </p>
              </div>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs font-bold">
            ...
            <span className="px-3 py-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-700 dark:text-emerald-400">
              With GSTIN:{" "}
              <strong className="font-mono">{metrics.withGst}</strong>
            </span>
            <span className="px-3 py-1.5 rounded-lg bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 text-rose-700 dark:text-rose-400">
              Missing GSTIN:{" "}
              <strong className="font-mono">{metrics.missingGst}</strong>
            </span>
            <span className="px-3 py-1.5 rounded-lg bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 text-indigo-700 dark:text-indigo-400">
              Mapped State:{" "}
              <strong className="font-mono">{metrics.withState}</strong>
            </span>
          </div>
        </header>
            ...
            <div className="md:col-span-3 flex items-center gap-2">
              ...
              >
                <option value="standard">
                  Enhanced Multi-Fetch (With Sub-lists)
                </option>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* Primary Table Pane */}
          <div
            className={`${selectedLedgerForInspect ? "lg:col-span-7" : "lg:col-span-12"} transition-all`}
          >
                ...
                <table className="w-full text-left border-collapse min-w-175">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-white/5 text-xs">
                    {displayedDebtors.map((item, idx) => {
                      ...
                      const isSelected =
                        selectedLedgerForInspect?.name === item.name;
                          ...
                          <td className="py-3 px-3">
                            {hasGst ? (
                              <div className="flex items-center gap-1.5">
                                ...
                                <button
                                  onClick={() =>
                                    copyToClipboard(item.gstin, `gst-${idx}`)
                                  }
                              ...
                              <div className="space-y-0.5">
                                <span className="inline-flex items-center gap-1 text-[10px] font-bold text-amber-600 bg-amber-50 dark:bg-amber-500/10 dark:text-amber-400 px-2 py-0.5 rounded">
                                  <Sparkles className="w-3 h-3" /> Prefix:{" "}
                                  {item.inferredGstPrefix}...
                                </span>
            ...
            <div className="lg:col-span-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl p-5 shadow-sm space-y-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-mono">
                  Parent: {selectedLedgerForInspect.parent} | Detection:{" "}
                  {selectedLedgerForInspect.gstinSource}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-[11px] font-mono">
                <div className="p-2 rounded bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-white/5">
                  ...
                  <strong
                    className={
                      selectedLedgerForInspect.gstin
                ...
                <div className="p-2 rounded bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-white/5">
                  ...
                  <strong
                    className={
                      selectedLedgerForInspect.inferredGstPrefix
                        ? "text-amber-500"
                        : "text-slate-400"
                    }
                  >
                ...
                <div className="p-2 rounded bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-white/5">
                  ...
                  <strong
                    className={
                      selectedLedgerForInspect.stateName
