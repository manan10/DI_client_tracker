...
const TallyTest = () => {
  ...
  const fetchCompanies = async () => {
    ...
    try {
      ...
      const matches = [
        ...responseData.matchAll(/<NAME[^>]*>(.*?)<\/NAME>/g),
      ].map((m) => m[1]);
  ...
  const fetchLedgers = async () => {
    ...
    try {
      ...
      const mapped = ledgerMatches.map((m) => {
        ...
        const addressList = addressMatches.map((a) =>
          tallyTemplates.unescapeXml(a[1]),
        );
        ...
        return {
          ...
          stateName: stateMatch
            ? tallyTemplates.unescapeXml(stateMatch[1])
            : "",
          country: countryMatch
            ? tallyTemplates.unescapeXml(countryMatch[1])
            : "India",
      ...
      setLedgers(mapped);
      logActivity(
        `Sync Complete: ${mapped.length} accounts mapped.`,
        "success",
      );
    } catch (err) {
  ...
  const postVoucher = async () => {
    ...
    try {
      const responseData = await request("/tally/proxy", "POST", {
        xml: displayXml,
      });
      setLastResponse(responseData);
      if (
        responseData.includes("<CREATED>1</CREATED>") ||
        responseData.includes("CREATED: 1")
      ) {
        ...
        <div className="lg:col-span-5 space-y-6 overflow-y-auto pr-4 custom-scrollbar">
          <div className="bg-white border border-slate-200 rounded-3xl shadow-sm p-6 space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">
                01. Bridge Setup
              </h2>
              <button
                onClick={fetchCompanies}
                className="flex items-center gap-2 bg-indigo-50 text-indigo-700 px-4 py-2 rounded-xl text-xs font-bold hover:bg-indigo-100 transition-all"
              >
                <RefreshCw
            ...
            <div
              className={`space-y-4 ${companies.length === 0 ? "opacity-30" : ""}`}
            >
              <select
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-3 p-3 rounded-xl border text-[10px] font-black uppercase transition-all ${isCompanyOpen ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-slate-50 text-slate-400"}`}
                >
                  {isCompanyOpen ? (
                ...
                <button
                  ...
                  disabled={!isCompanyOpen}
                  className="flex-1 bg-slate-900 text-white p-3 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2"
                >
          ...
          <div
            className={`bg-white border border-slate-200 rounded-3xl shadow-sm p-6 transition-all ${ledgers.length === 0 ? "opacity-20 pointer-events-none" : ""}`}
          >
            <h2 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-6">
              02. New Entry
            </h2>
            <div className="flex bg-slate-100 p-1 rounded-xl mb-6">
              {["Receipt", "Payment", "Sales"].map((t) => (
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${voucherType === t ? "bg-white text-indigo-600 shadow-sm" : "text-slate-400"}`}
                >
                  {t}
                </button>
              ))}
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) => handleLedgerSelect(e.target.value)}
                >
                  <option value="">
                    {voucherType === "Sales"
                      ? "-- Choose Party (Debtor) --"
              ...
              {voucherType === "Sales" ? (
                <>
                  ...
                  <select
                    className="w-full bg-indigo-50 border border-indigo-100 p-3 rounded-xl text-xs font-bold text-indigo-700 outline-none"
                    value={voucherData.incomeLedger}
                    onChange={(e) =>
                      handleFormChange({ incomeLedger: e.target.value })
                    }
                  >
                    <option value="">-- Choose Sales/Income Ledger --</option>
                    <option value="MF COMMISION INCOME">
                      MF COMMISION INCOME
                    </option>
                    <option value="MF COMMISION (IGST)">
                      MF COMMISION (IGST)
                    </option>
                    <option value="MF COMMISSION (LOC)">
                      MF COMMISSION (LOC)
                    </option>
                    {ledgers
                      .filter(
                  ...
                  <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-4">
                    <div className="flex gap-4">
                      <select
                        className="bg-white border border-slate-200 p-2 rounded-lg text-xs font-bold outline-none w-1/3"
                      ...
                      >
                        ...
                        <option value="INTERSTATE">
                          GST ARN (Interstate 18%)
                        </option>
                      </select>
                      <input
                        type="number"
                        placeholder="Base Commission Amt"
                        className="flex-1 bg-white border border-slate-200 p-2 rounded-lg text-sm font-black text-indigo-600 outline-none"
                      ...
                      <div className="grid grid-cols-2 gap-4 animate-in fade-in zoom-in duration-200">
                        <div className="space-y-1">
                          <input
                            ...
                            value={voucherData.cgstLedger}
                            onChange={(e) =>
                              handleFormChange({ cgstLedger: e.target.value })
                            }
                          />
                          <input
                            type="number"
                            readOnly
                            className="w-full bg-slate-100 border border-slate-200 p-2 rounded-lg text-xs font-bold text-slate-600 outline-none cursor-not-allowed"
                            value={voucherData.cgstAmount}
                        ...
                        <div className="space-y-1">
                          <input
                            type="text"
                            className="w-full bg-transparent text-[10px] font-bold text-slate-400 outline-none"
                            value={voucherData.sgstLedger}
                            onChange={(e) =>
                              handleFormChange({ sgstLedger: e.target.value })
                            }
                          />
                          <input
                            type="number"
                    ...
                    {voucherData.gstType === "INTERSTATE" && (
                      <div className="animate-in fade-in zoom-in duration-200 space-y-1">
                        <input
                          type="text"
                          className="w-full bg-transparent text-[10px] font-bold text-slate-400 outline-none"
                          value={voucherData.igstLedger}
                          onChange={(e) =>
                            handleFormChange({ igstLedger: e.target.value })
                          }
                        />
                        <input
                          type="number"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <select
                      ...
                      onChange={(e) =>
                        handleFormChange({ bankAccount: e.target.value })
                      }
                    >
                      ...
                      {ledgers
                        .filter(
                          (l) =>
                            l.parent.includes("Bank") ||
                            l.parent.includes("Cash"),
                        )
                        .map((l) => (
                          <option key={l.name} value={l.name}>
                            {l.name}
                  ...
                  <input
                    type="number"
                    placeholder="Amount"
                    className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-sm font-black text-indigo-600 outline-none"
                    value={voucherData.amount}
                    onChange={(e) =>
              ...
              <textarea
                rows="2"
                className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-xs font-medium outline-none"
          ...
          <div className="bg-white border border-slate-200 rounded-4xl shadow-xl flex flex-col overflow-hidden h-3/5 relative">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
              <div className="flex gap-6">
                <button
                  ...
                  className={`text-[10px] font-black uppercase tracking-widest pb-1 transition-all ${viewMode === "request" ? "text-indigo-600 border-b-2 border-indigo-600" : "text-slate-400"}`}
                >
                  Request XML
                </button>
                <button
                  onClick={() => setViewMode("response")}
                  className={`text-[10px] font-black uppercase tracking-widest pb-1 transition-all ${viewMode === "response" ? "text-indigo-600 border-b-2 border-indigo-600" : "text-slate-400"}`}
                >
                  Response Feedback
                </button>
              </div>
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={() =>
                    copyToClipboard(
                      viewMode === "request" ? displayXml : lastResponse,
                      "data",
                    )
                  }
                  className="flex items-center gap-2 px-3 py-1.5 hover:bg-slate-100 rounded-lg transition-all border border-transparent hover:border-slate-200 group"
                >
                  {copiedSection === "data" ? (
                  ...
                  <span className="text-[9px] font-bold text-slate-400 group-hover:text-slate-600 uppercase">
                    Copy
                  </span>
                </button>
                <button
                  onClick={postVoucher}
                  disabled={loading}
                  className="bg-blue-600 text-white px-5 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 hover:bg-blue-700 active:scale-95 transition-all"
                >
            ...
            <textarea
              ...
              value={
                viewMode === "request"
                  ? displayXml
                  : lastResponse || "// Feedback will appear here..."
              }
              onChange={(e) => {
                if (viewMode === "request") {
          ...
          <div className="bg-white border border-slate-200 rounded-4xl shadow-sm flex flex-col overflow-hidden h-2/5 relative">
            <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <span className="text-[9px] font-black uppercase tracking-widest text-slate-500 flex items-center gap-2">
                <Activity className="w-3.5 h-3.5 text-indigo-500" /> Connection
                Activity
              </span>
              <button
                onClick={() =>
                  copyToClipboard(
                    activityLog.map((l) => `[${l.time}] ${l.msg}`).join("\n"),
                    "feed",
                  )
                }
            ...
            <div className="flex-1 p-5 overflow-y-auto space-y-3">
              {activityLog.map((log, i) => (
                ...
                >
                  <span className="text-[9px] font-bold text-slate-300 mt-0.5 whitespace-nowrap">
                    {log.time}
                  </span>
                  <span
                    className={`text-xs font-semibold ${log.type === "error" ? "text-red-500" : log.type === "success" ? "text-emerald-600" : "text-slate-600"}`}
                  >
                    {log.msg}
                  </span>
                </div>
