...
const APP_ENTITLEMENTS = [
  { 
    id: 'CLIENT_TRACKER', 
    label: 'Distributor Portal (CRM)',
    icon: LayoutGrid,
    theme: {
      active: 'bg-indigo-50 dark:bg-indigo-500/10 border-indigo-200 dark:border-indigo-500/30 text-indigo-700 dark:text-indigo-400',
      checkbox: 'bg-indigo-600 border-indigo-600',
      pill: 'bg-indigo-50 dark:bg-indigo-500/10 border-indigo-100 dark:border-indigo-500/20 text-indigo-700 dark:text-indigo-300'
    }
  },
  { 
    id: 'EXPENSE_TRACKER', 
    label: 'Family Expense Tracker',
    icon: LayoutGrid,
    theme: {
      active: 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400',
  ...
  { 
    id: 'SHOW_RATER', 
    label: 'Show Rater (TV Vault)',
    icon: Clapperboard,
    theme: {
      active: 'bg-rose-50 dark:bg-rose-500/10 border-rose-200 dark:border-rose-500/30 text-rose-700 dark:text-rose-400',
      checkbox: 'bg-rose-600 border-rose-600',
      pill: 'bg-rose-50 dark:bg-rose-500/10 border-rose-100 dark:border-rose-500/20 text-rose-700 dark:text-rose-300'
    }
  }
];
...
const UserManagement = () => {
  ...
  const getAppPillStyle = (appKey) => {
    const matched = APP_ENTITLEMENTS.find(a => a.id === appKey);
    return matched ? matched.theme.pill : 'bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700';
  };
          ...
          <div key={u._id} className="bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-white/5 rounded-md p-4 shadow-sm flex flex-col gap-4">
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 p-2.5 rounded border border-slate-100 dark:border-white/5 flex flex-wrap gap-1.5">
              {u.allowedApps?.length > 0 ? u.allowedApps.map(app => (
                <span key={app} className={`px-2 py-1 border rounded-sm text-[9px] font-bold uppercase tracking-widest shadow-sm ${getAppPillStyle(app)}`}>
                  {app.replace('_', ' ')}
                </span>
                ...
                <tr key={u._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-2">
                      {u.allowedApps?.length > 0 ? u.allowedApps.map(app => {
                        const ent = APP_ENTITLEMENTS.find(a => a.id === app);
                        const Icon = ent?.icon || LayoutGrid;
                        return (
                          <span key={app} className={`flex items-center gap-1 px-2 py-1 border rounded-sm text-[9px] font-bold uppercase tracking-widest shadow-sm ${getAppPillStyle(app)}`}>
                            <Icon size={10} /> {app.replace('_', ' ')}
                          </span>
                        );
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {APP_ENTITLEMENTS.map(app => {
                    ...
                    return (
                      ...
                      >
                        <div className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 ${hasAccess ? `${app.theme.checkbox} text-white` : 'border-slate-300 dark:border-slate-600 text-transparent'}`}>
                          <Check size={12} strokeWidth={4} />
                        </div>
                        <AppIcon size={14} className="shrink-0" />
