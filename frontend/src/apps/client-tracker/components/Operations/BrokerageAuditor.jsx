...
const BrokerageAuditor = () => {
  ...
  const handleFileChange = (e, setter, typeName) => {
    ...
    if (
      ...
      toast.error(
        `Please upload a valid Excel or CSV file for the ${typeName}.`,
      );
    }
  };
  ...
  const handleReconcile = async () => {
    ...
    try {
      ...
      if (response.unpaidFolios.length === 0) {
        toast.success(
          `100% Payout Success! Brokerage received for all ${response.paidFolios.length} folios.`,
        );
        setActiveTab("paid");
      } else {
        toast.warning(
          `Identified ${response.unpaidFolios.length} folios missing brokerage payments.`,
        );
        setActiveTab("unpaid");
      }
    } catch (error) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
              Brokerage Revenue Auditor
            </h1>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Cross-reference AMC payout sheets against your master folio list
              to track unpaid commissions.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* CAMS Upload */}
        <div
          className={`relative border-2 border-dashed rounded-xl p-8 text-center transition-colors ${camsFile ? "border-amber-500 bg-amber-50 dark:bg-amber-900/10" : "border-slate-300 dark:border-slate-700 hover:border-amber-400"}`}
        >
          <input
            type="file"
            accept=".xlsx, .xls, .csv"
            onChange={(e) => handleFileChange(e, setCamsFile, "Master File")}
          ...
          <div className="flex flex-col items-center space-y-3 pointer-events-none">
            ...
            <div>
              <h3 className="font-semibold text-slate-900 dark:text-white">
                CAMS/KARVY Master List
              </h3>
              <p className="text-sm text-slate-500 mt-1">
                {camsFile ? camsFile.name : "Upload Master Source of Truth"}
              </p>
            </div>
          </div>
        </div>
        ...
        <div
          className={`relative border-2 border-dashed rounded-xl p-8 text-center transition-colors ${brokerageFile ? "border-emerald-500 bg-emerald-50 dark:bg-emerald-900/10" : "border-slate-300 dark:border-slate-700 hover:border-emerald-400"}`}
        >
          <input
            type="file"
            accept=".xlsx, .xls, .csv"
            onChange={(e) =>
              handleFileChange(e, setBrokerageFile, "Brokerage List")
            }
            className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
          />
          <div className="flex flex-col items-center space-y-3 pointer-events-none">
            ...
            <div>
              <h3 className="font-semibold text-slate-900 dark:text-white">
                Brokerage Payout List
              </h3>
              <p className="text-sm text-slate-500 mt-1">
                {brokerageFile
                  ? brokerageFile.name
                  : "Upload list with Folios & Amounts"}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="flex items-center space-x-2 text-sm text-slate-500">
          ...
          <span>
            Groups multiple scheme payouts by Folio and checks against your
            Master list.
          </span>
        </div>
        <button
          ...
          disabled={
            !camsFile || !brokerageFile || !amcSheetName.trim() || loading
          }
        ...
        >
          {loading ? (
            <>
              ...
              <span>Auditing Revenues...</span>
            </>
          ) : (
            <>
              <Landmark className="w-5 h-5" />
              <span>Run Brokerage Audit</span>
            </>
          )}
        </button>
      </div>
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg text-sm flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs text-amber-500 font-semibold mt-1">
                      Sheet: {stats.cams.sheetTargeted}
                    </p>
                  </div>
                  <span className="text-xs font-bold text-amber-600 bg-amber-50 dark:bg-amber-500/10 px-2.5 py-1 rounded">
                    → {stats.cams.uniqueFolios} Expected Folios
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 p-2 gap-2">
              <div className="flex bg-slate-200/60 dark:bg-slate-900 rounded-lg p-1 space-x-1">
                <button
                  ...
                  className={`flex items-center space-x-2 px-4 py-2 rounded-md font-medium text-xs transition-all ${activeTab === "unpaid" ? "bg-white dark:bg-slate-800 shadow text-rose-600 dark:text-rose-400" : "text-slate-500 hover:text-slate-700"}`}
                >
                  <XSquare className="w-3.5 h-3.5" />
                  <span>Unpaid Folios ({unpaidData.length})</span>
                </button>
                <button
                  onClick={() => {
                    setActiveTab("paid");
                  ...
                  className={`flex items-center space-x-2 px-4 py-2 rounded-md font-medium text-xs transition-all ${activeTab === "paid" ? "bg-white dark:bg-slate-800 shadow text-emerald-600 dark:text-emerald-400" : "text-slate-500 hover:text-slate-700"}`}
                >
                  <CheckSquare className="w-3.5 h-3.5" />
                  <span>Paid Folios ({paidData.length})</span>
                </button>
              </div>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="text-xs text-slate-500 bg-slate-100 dark:bg-slate-800 dark:text-slate-400 sticky top-0 shadow-sm z-10">
                    <tr>
                      <th
                        ...
                        className="px-6 py-3 cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                      >
                      ...
                      >
                        <div className="flex items-center justify-end">
                          Brokerage Earned {renderSortIcon("brokerageAmount")}
                        </div>
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {processedTableData.map((row, rowIndex) => (
                      ...
                      >
                        <td className="px-6 py-3.5 whitespace-nowrap font-bold text-slate-900 dark:text-white">
                          {row.folio}
                        </td>
                        ...
                        <td className="px-6 py-3.5 whitespace-nowrap text-right font-mono font-medium">
                          {row.brokerageAmount > 0 ? (
                            <span className="text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-2 py-1 rounded">
                              {formatINR(row.brokerageAmount)}
                            </span>
                          ) : (
                            <span className="text-rose-500 bg-rose-50 dark:bg-rose-500/10 px-2 py-1 rounded font-bold">
                              UNPAID
                            </span>
                          )}
                        </td>
