...
const FolioReconciler = () => {
  ...
  const handleReconcile = async () => {
    ...
    if (!amcSheetName.trim()) {
      toast.error("Please specify the target CAMS/KARVY Sheet Name.");
      return;
    }
    ...
    try {
      ...
      if (response.missingFolios.length === 0) {
        toast.success(
          `Perfect Match! ${response.matchedFolios.length} unique folios verified.`,
        );
        setActiveTab("matched");
      } else {
        toast.warning(
          `Identified ${response.missingFolios.length} missing folios.`,
        );
        setActiveTab("missing");
      }
    } catch (error) {
  ...
  // ==========================================
  // EXPORT ENGINE: SMART FORMATTING (NO HEADERS)
  // ==========================================
  const exportToCamsFormat = () => {
    ...
    // Enhanced fuzzy matching to accommodate both CAMS and KARVY template structures
    const colIndexes = {
      folio: camsHeaders.findIndex(
        (h) => norm(h).includes("folio") && !norm(h).includes("portfolio"),
      ),
      name: camsHeaders.findIndex(
        (h) => norm(h).includes("invname") || norm(h).includes("investor"),
      ),
      ...
      amcCode: camsHeaders.findIndex(
        (h) => norm(h).includes("amccode") || norm(h).includes("productcode"),
      ),
      amcName: camsHeaders.findIndex(
        (h) => norm(h).includes("amcname") || norm(h) === "fund",
      ),
      brokerCode: camsHeaders.findIndex(
        (h) => norm(h).includes("brokercode") || norm(h).includes("arn"),
      ),
    };

    // Start with an empty array. NO HEADERS ALLOWED.
    const exportData = [];
    ...
    missingData.forEach((row) => {
      ...
      if (colIndexes.folio !== -1)
        newRow[colIndexes.folio] = row.folio !== "-" ? row.folio : "";
      if (colIndexes.name !== -1)
        newRow[colIndexes.name] = row.name !== "-" ? row.name : "";
      if (colIndexes.pan !== -1)
        newRow[colIndexes.pan] = row.pan !== "-" ? row.pan : "";
      if (colIndexes.email !== -1)
        newRow[colIndexes.email] = row.email !== "-" ? row.email : "";
      if (colIndexes.mobile !== -1)
        newRow[colIndexes.mobile] = row.mobile !== "-" ? row.mobile : "";

      // Map Static Template Constants
      if (colIndexes.amcCode !== -1)
        newRow[colIndexes.amcCode] = camsConstants.amcCode || "";
      if (colIndexes.amcName !== -1)
        newRow[colIndexes.amcName] = camsConstants.amcName || "";
      if (colIndexes.brokerCode !== -1)
        newRow[colIndexes.brokerCode] = camsConstants.brokerCode || "";

      exportData.push(newRow);
    });

    // Generate and download the header-less file
    const worksheet = XLSX.utils.aoa_to_sheet(exportData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(
      workbook,
      worksheet,
      amcSheetName || "Missing_Folios",
    );
    XLSX.writeFile(
      workbook,
      `Copy_Ready_Folios_${amcSheetName || "Target"}.xlsx`,
    );

    toast.success(
      "Ready! You can copy the entire file and paste directly at the bottom of your original sheet.",
    );
  };
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
              Master Folio Reconciler
            </h1>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Identify missing folios by checking Combined WE exports against a
              targeted CAMS/KARVY sheet.
            </p>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
        ...
        <input
          type="text"
          value={amcSheetName}
          onChange={(e) => setAmcSheetName(e.target.value)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div
          className={`relative border-2 border-dashed rounded-xl p-8 text-center transition-colors ${weFiles.length > 0 ? "border-emerald-500 bg-emerald-50 dark:bg-emerald-900/10" : "border-slate-300 dark:border-slate-700 hover:border-emerald-400"}`}
        >
          <input
            type="file"
            accept=".xlsx, .xls, .csv"
            multiple
            onChange={handleWeFilesChange}
          ...
          <div className="flex flex-col items-center space-y-3 pointer-events-none">
            ...
            <div>
              <h3 className="font-semibold text-slate-900 dark:text-white">
                Combined Wealth Elite Data
              </h3>
              <p className="text-sm text-slate-500 mt-1">
                {weFiles.length > 0
                  ? `${weFiles.length} file(s) selected`
                  : "Upload WE exports"}
              </p>
            </div>
          </div>
        ...
        <div
          className={`relative border-2 border-dashed rounded-xl p-8 text-center transition-colors ${camsFile ? "border-emerald-500 bg-emerald-50 dark:bg-emerald-900/10" : "border-slate-300 dark:border-slate-700 hover:border-emerald-400"}`}
        >
          <input
            type="file"
            accept=".xlsx, .xls, .csv"
            onChange={handleCamsFileChange}
            className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
          />
          <div className="flex flex-col items-center space-y-3 pointer-events-none">
            {camsFile ? (
            ...
            <div>
              <h3 className="font-semibold text-slate-900 dark:text-white">
                CAMS/KARVY Master Report
              </h3>
              <p className="text-sm text-slate-500 mt-1">
                {camsFile ? camsFile.name : "Upload Master Report"}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="flex items-center space-x-2 text-sm text-slate-500">
          ...
          <span>
            Automatically maps matched PANs to your Client CRM Database to
            enrich email & mobile records.
          </span>
        </div>
        <button
          onClick={handleReconcile}
          disabled={
            !camsFile || weFiles.length === 0 || !amcSheetName.trim() || loading
          }
        ...
        >
          {loading ? (
            <>
              ...
              <span>Processing DB Lookup...</span>
            </>
          ) : (
            <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5">
              <h3 className="text-xs font-bold uppercase text-slate-500 mb-4">
                Wealth Elite Configuration Pool
              </h3>
              <div className="space-y-3 max-h-32 overflow-y-auto pr-2">
                {stats.wealthElite.map((f, i) => (
                  <div
                    key={i}
                    className="flex justify-between items-center bg-slate-50 dark:bg-slate-800/50 p-2.5 rounded-lg text-sm"
                  >
                    <span className="font-medium text-slate-700 dark:text-slate-300 truncate pr-4">
                      {f.fileName}
                    </span>
                    <span className="text-xs font-semibold text-slate-500 bg-slate-200 dark:bg-slate-700 px-2 py-0.5 rounded">
                      {f.rawRowsParsed} Rows
                    </span>
                  </div>
                ))}
              </div>
              <div className="mt-4 pt-3 border-t border-slate-200 dark:border-slate-700 flex justify-between text-sm">
                <span className="font-bold text-slate-700 dark:text-slate-300">
                  Unified WE Unique Count
                </span>
                <span className="font-bold text-emerald-600 dark:text-emerald-400">
                  {stats.overall.totalWeUniqueCombined} Folios
                </span>
              </div>
            </div>
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg text-sm flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs text-rose-500 font-semibold mt-1">
                      Target Sheet: {stats.cams.sheetTargeted}
                    </p>
                  </div>
                  <span className="text-xs font-bold text-rose-600 bg-rose-50 dark:bg-rose-500/10 px-2.5 py-1 rounded">
                    → {stats.cams.uniqueFolios} Unique
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 p-2 gap-2">
              <div className="flex bg-slate-200/60 dark:bg-slate-900 rounded-lg p-1 space-x-1">
                <button
                  ...
                  className={`flex items-center space-x-2 px-4 py-2 rounded-md font-medium text-xs transition-all ${activeTab === "missing" ? "bg-white dark:bg-slate-800 shadow text-rose-600 dark:text-rose-400" : "text-slate-500 hover:text-slate-700"}`}
                >
                  <XSquare className="w-3.5 h-3.5" />
                  <span>Missing from Target ({missingData.length})</span>
                </button>
                <button
                  onClick={() => {
                    setActiveTab("matched");
                  ...
                  className={`flex items-center space-x-2 px-4 py-2 rounded-md font-medium text-xs transition-all ${activeTab === "matched" ? "bg-white dark:bg-slate-800 shadow text-emerald-600 dark:text-emerald-400" : "text-slate-500 hover:text-slate-700"}`}
                >
                  <CheckSquare className="w-3.5 h-3.5" />
                  <span>Matched ({matchedData.length})</span>
                </button>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                {activeTab === "missing" && missingData.length > 0 && (
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">Copy-Ready Export</span>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="text-xs text-slate-500 bg-slate-100 dark:bg-slate-800 dark:text-slate-400 sticky top-0 shadow-sm z-10">
                    <tr>
                      <th
                        ...
                        className="px-6 py-3 whitespace-nowrap cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                      >
                        <div className="flex items-center">
                          Folio No. {renderSortIcon("folio")}
                        </div>
                      </th>
                      <th
                  ...
                  <tbody>
                    {processedTableData.map((row, rowIndex) => (
                      <tr
                        key={rowIndex}
                        className="bg-white dark:bg-slate-900 border-b dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors"
                      >
                        ...
                        <td className="px-6 py-3.5 whitespace-nowrap text-slate-700 dark:text-slate-300 tracking-wider uppercase font-mono text-xs">
                          {row.pan}
                        </td>
                        <td className="px-6 py-3.5 whitespace-nowrap text-slate-600 dark:text-slate-400 text-xs">
                          {row.email}
                        </td>
                        <td className="px-6 py-3.5 whitespace-nowrap text-slate-600 dark:text-slate-400 font-mono text-xs">
                          {row.mobile}
                        </td>
                      </tr>
                    ))}
