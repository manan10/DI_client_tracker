...
const ArnTransferReconciler = () => {
  ...
  const handleCamsFileChange = (e) => {
    ...
    if (
      ...
      toast.error(
        "Please upload a valid Excel or CSV file for the Old Master Report.",
      );
    }
  };
  ...
  const handleReconcile = async () => {
    ...
    try {
      ...
      if (response.missingFolios.length === 0) {
        toast.success(
          `100% Transfer Success! All ${response.matchedFolios.length} folios migrated successfully.`,
        );
        setActiveTab("matched");
      } else {
        toast.warning(
          `Identified ${response.missingFolios.length} folios that failed to transfer.`,
        );
        setActiveTab("missing");
      }
    } catch (error) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
              ARN Transfer Auditor
            </h1>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Verify that 100% of your old master folios successfully migrated
              to the new Transferee ARN.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div
          className={`relative border-2 border-dashed rounded-xl p-8 text-center transition-colors ${camsFile ? "border-blue-500 bg-blue-50 dark:bg-blue-900/10" : "border-slate-300 dark:border-slate-700 hover:border-blue-400"}`}
        >
          <input
            type="file"
            accept=".xlsx, .xls, .csv"
          ...
          <div className="flex flex-col items-center space-y-3 pointer-events-none">
            ...
            <div>
              <h3 className="font-semibold text-slate-900 dark:text-white">
                Old ARN Master List (Transferor)
              </h3>
              <p className="text-sm text-slate-500 mt-1">
                {camsFile ? camsFile.name : "Upload Original CAMS/KARVY Master"}
              </p>
            </div>
          </div>
        </div>
        ...
        <div
          className={`relative border-2 border-dashed rounded-xl p-8 text-center transition-colors ${weFiles.length > 0 ? "border-purple-500 bg-purple-50 dark:bg-purple-900/10" : "border-slate-300 dark:border-slate-700 hover:border-purple-400"}`}
        >
          <input
            type="file"
            accept=".xlsx, .xls, .csv"
          ...
          <div className="flex flex-col items-center space-y-3 pointer-events-none">
            {weFiles.length > 0 ? (
              <Files className="w-10 h-10 text-purple-500" />
            ) : (
              <Upload className="w-10 h-10 text-slate-400" />
            )}
            <div>
              <h3 className="font-semibold text-slate-900 dark:text-white">
                New ARN Transferred Data (Transferee)
              </h3>
              <p className="text-sm text-slate-500 mt-1">
                {weFiles.length > 0
                  ? `${weFiles.length} file(s) selected`
                  : "Upload Multiple WE Scheme Files"}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="flex items-center space-x-2 text-sm text-slate-500">
          ...
          <span>
            Cross-references Transferred data against the Original Master list.
          </span>
        </div>
        <button
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 flex flex-col justify-between border-t-4 border-t-blue-500">
              <div>
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg text-sm flex justify-between items-center">
                  ...
                  <span className="text-xs font-bold text-blue-600 bg-blue-50 dark:bg-blue-500/10 px-2.5 py-1 rounded">
                    → {stats.cams.uniqueFolios} Master Folios
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 p-2 gap-2">
              <div className="flex bg-slate-200/60 dark:bg-slate-900 rounded-lg p-1 space-x-1">
                ...
                >
                  ...
                  <span>Failed Transfers ({missingData.length})</span>
                </button>
                <button
                  onClick={() => {
                    setActiveTab("matched");
                ...
                >
                  ...
                  <span>Successful Transfers ({matchedData.length})</span>
                </button>
              </div>
