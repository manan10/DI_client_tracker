...
import { Layers, Keyboard, Check, Edit3 } from "lucide-react";
...
const SalesStep = ({
  ...
  const activeArnId = useMemo(() => {
    ...
    if (typeof raw === "object" && raw !== null)
      return raw._id || raw.arnCode || raw.arn;
    return raw;
  ...
  const companyLedgers = useMemo(() => {
    return allCompanyLedgers.filter((l) =>
  ...
  const salesTransactions = useMemo(() => {
    return localTransactions
      ...
      .map((tx) => {
        ...
        const isLocalAmc =
          activeSalesLedger.toUpperCase().includes("NJ") ||
        ...
        let cgst = 0,
          sgst = 0,
          igst = 0,
          grossVoucherTotal = netAmount;

        const applyCGST =
          tx.applyCGST !== undefined && tx.applyCGST !== null
            ? isTrue(tx.applyCGST)
            : isGstCompliant && isLocalAmc;
        const applySGST =
          tx.applySGST !== undefined && tx.applySGST !== null
            ? isTrue(tx.applySGST)
            : isGstCompliant && isLocalAmc;
        const applyIGST =
          tx.applyIGST !== undefined && tx.applyIGST !== null
            ? isTrue(tx.applyIGST)
            : isGstCompliant && !isLocalAmc;
  ...
  const availableBanks = useMemo(() => {
    const banks = [
      ...new Set(salesTransactions.map((t) => t.bank).filter(Boolean)),
    ];
  ...
  const displayTransactions = useMemo(() => {
    let filtered = salesTransactions.filter(
      (t) => t.bank === currentBank || !t.bank,
    );
  ...
  const currentTxId = useMemo(() => {
    if (selectedTxId && displayTransactions.some((t) => t._id === selectedTxId))
      return selectedTxId;
  ...
  const globalTotals = useMemo(() => {
    return salesTransactions.reduce(
      (acc, curr) => acc + curr.grossVoucherTotal,
      0,
    );
  }, [salesTransactions]);
        ...
        const updated = prev.map((t) => {
          if (t._id === txId) {
            const isConfigChange =
              "individualSalesLedger" in payload ||
  ...
  const getLockedTaxPayload = useCallback(
    (tx) => ({
  ...
  const handleVerifyAndNext = useCallback(() => {
    ...
    const currentIndex = displayTransactions.findIndex(
      (t) => t._id === currentTxId,
    );
  ...
  const handleSelectAll = async () => {
    ...
    const isAllSelected = displayTransactions.every((tx) =>
      isTrue(tx.isSalesApproved),
    );
    ...
    toast.success(
      targetState
        ? `Approved all sales in view`
        : `Unapproved all sales in view`,
    );
  ...
  const handleLedgerSelect = async (ledgerName) => {
    if (ledgerModalMode === "GLOBAL" || !globalSalesLedger) {
      ...
      if (selection.audit?._id) {
        try {
          await request(`/audit/${selection.audit._id}`, "PUT", {
            salesIncomeLedger: ledgerName,
          });
        } catch {
          // Silent catch
        }
  ...
  const isAllDisplayedChecked =
    displayTransactions.length > 0 &&
    displayTransactions.every((tx) => isTrue(tx.isSalesApproved));
      ...
      <div className="h-full w-full flex flex-col items-center justify-center p-6 text-center space-y-4 font-sans select-none">
        <div className="space-y-1 max-w-md">
          ...
          <p className="text-xs text-slate-500 leading-relaxed">
            There are no transactions marked as sales/commission for this month.
            You can proceed directly to the batch summary.
          </p>
        </div>
      ...
      <div className="h-full w-full flex flex-col items-center justify-center p-6 bg-slate-50/50 dark:bg-slate-900/30 font-sans">
        <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-2xl p-6 lg:p-8 border border-slate-200 dark:border-white/10 shadow-xl space-y-6 animate-in zoom-in-95 duration-200">
          <div className="space-y-2 text-center">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Before inspecting the {salesTransactions.length} sales vouchers,
              select the default income account for{" "}
          ...
          <div className="px-4 lg:px-6 py-2.5 flex items-center justify-between gap-3 min-w-0">
            <div className="flex items-center gap-3 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {isGstCompliant ? (
                  <span className="px-1.5 py-0.5 rounded text-[8px] font-black uppercase bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                    GST Compliant
                  </span>
                ) : (
                  <span className="px-1.5 py-0.5 rounded text-[8px] font-black uppercase bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                    Non-GST
                  </span>
                )}
              </div>
          ...
          <section className="flex-1 flex flex-col bg-white dark:bg-[#07090E] relative min-w-0 overflow-hidden">
            <div className="px-4 py-2 border-b border-slate-200/80 dark:border-white/10 bg-slate-50/60 dark:bg-slate-800/40 flex items-center justify-between shrink-0">
              ...
              >
                ...
                <span>
                  {displayTransactions.length} Vouchers in {currentBank}
                </span>
                <kbd className="hidden sm:inline-block px-1.5 py-0.2 rounded bg-slate-200 dark:bg-slate-700 text-[9px] font-mono">
                  A
                </kbd>
              </button>
