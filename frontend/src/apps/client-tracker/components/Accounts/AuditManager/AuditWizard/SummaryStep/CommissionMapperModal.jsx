...
const CommissionMapperModal = ({
  ...
  const findBestLiveAMCMatch = (ledgerName, activeAmcList) => {
    ...
    for (const amcObj of activeAmcList) {
      ...
      if (
        normalizedLedger.includes(cleanAmcToken) ||
        cleanAmcToken.includes(normalizedLedger)
      ) {
        return amcName;
      }
    }
  ...
  const handleCommitPayload = async () => {
    ...
    try {
      ...
      if (response?.success) {
        ...
        throw new Error(
          response?.error || "Transaction could not be completed.",
        );
      }
    } catch (err) {
          ...
          <div className="px-5 lg:px-8 py-5 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-[#0F172A] shrink-0 relative overflow-hidden">
            ...
            <div className="space-y-1 z-10">
              ...
              <p className="text-[10px] lg:text-[11px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest flex items-center gap-2 pl-1">
                <Landmark size={14} /> Auto-map bank statement ledgers to master
                AMC profiles and log advisory commissions in one click.
              </p>
            </div>
            {!isSaving && (
              <button
                onClick={onClose}
                className="z-10 p-2 text-slate-400 hover:text-slate-700 dark:hover:text-white rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-all active:scale-95"
              >
            ...
            <div className="flex-1 p-6 lg:p-8 flex flex-col justify-between overflow-hidden select-none relative bg-white/50 dark:bg-transparent">
              ...
              <div className="space-y-6 max-w-2xl mx-auto w-full pt-12 z-10">
                <div className="flex items-center justify-center gap-3 pb-6">
                  {!executionComplete ? (
                    <div className="relative">
                      ...
                      <Loader2
                        className="animate-spin text-blue-600 dark:text-indigo-400 shrink-0 relative z-10"
                        size={40}
                      />
                    </div>
                  ) : (
                ...
                <div className="space-y-5 max-w-md mx-auto text-left text-[12px] lg:text-[13px] font-bold text-slate-600 dark:text-slate-400">
                  {executionLogs.map((log, i) => (
                    <p
                      key={i}
                      className="tracking-wide flex items-start gap-4 animate-in slide-in-from-bottom-2 fade-in duration-300"
                    >
              ...
              <div className="max-w-md mx-auto w-full mt-8 space-y-3 shrink-0 pb-12 z-10">
                <div className="flex justify-between items-end font-black text-[11px] text-slate-400 uppercase tracking-widest">
                  ...
                  <span className="tabular-nums text-blue-600 dark:text-indigo-400">
                    {executionProgress}%
                  </span>
                </div>
              ...
              {!loadingAmcs && mappings.length > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6 p-4 lg:px-8 lg:py-5 shrink-0 bg-white/60 dark:bg-[#0B1120] border-b border-slate-200 dark:border-slate-800 z-10">
                  <div className="bg-white dark:bg-[#111827] border border-blue-100 dark:border-indigo-500/20 p-5 rounded-2xl flex flex-col gap-1.5 shadow-sm hover:shadow-md transition-all relative overflow-hidden">
                    ...
                    <span className="text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-2.5">
                      <Coins
                        size={14}
                    ...
                    <span className="text-xl lg:text-2xl font-[1000] text-slate-800 dark:text-white tabular-nums tracking-tight">
                      {formatINR(totalBase)}
                    </span>
                  </div>
                  ...
                  <div className="bg-emerald-50 dark:bg-[#0B1813] border border-emerald-200 dark:border-emerald-900/50 p-5 rounded-2xl flex flex-col gap-1.5 relative overflow-hidden shadow-sm hover:shadow-md transition-all">
                    ...
                    <span className="text-[11px] font-black text-emerald-800 dark:text-emerald-500 uppercase tracking-widest flex items-center gap-2.5">
                      <TrendingUp size={14} /> Gross Settled
                    </span>
                    <span className="text-xl lg:text-2xl font-[1000] text-emerald-600 dark:text-emerald-400 tabular-nums tracking-tight">
              ...
              {loadingAmcs ? (
                <div className="flex-1 flex flex-col items-center justify-center gap-5 opacity-60 bg-[#F8FAFC] dark:bg-[#0B1120]">
                  <Loader2
                    ...
                    size={32}
                  />
                  <p className="text-[11px] font-black uppercase tracking-[0.2em] text-center text-slate-500">
                ...
                <>
                        ...
                        >
                          <div className="flex justify-between items-start gap-3">
                            <div className="flex flex-col gap-1">
                              ...
                              <span className="text-[13px] font-black uppercase tracking-tight text-slate-800 dark:text-white leading-tight">
                                {m.ledgerName}
                              </span>
                            </div>
                            <div className="flex flex-col items-end shrink-0">
                              <span className="text-[16px] font-[1000] text-emerald-600 dark:text-emerald-400 tabular-nums leading-none tracking-tight">
                                {formatINR(m.totalAmount).replace("₹", "")}
                              </span>
                          ...
                          <div className="grid grid-cols-2 gap-2 bg-slate-50 dark:bg-slate-800/50 p-3.5 rounded-xl border border-slate-100 dark:border-slate-800">
                            <div className="flex flex-col px-1">
                              <span className="text-[9px] font-black text-blue-600/70 dark:text-indigo-400/70 uppercase tracking-widest">
                                Base Amount
                              </span>
                            ...
                            <div className="flex flex-col px-3 border-l border-slate-200 dark:border-slate-700">
                              <span className="text-[9px] font-black text-purple-600/70 dark:text-purple-400/70 uppercase tracking-widest">
                                GST Collected
                              </span>
                              <span className="text-[13px] font-bold text-slate-800 dark:text-slate-200 tabular-nums">
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                {m.matchedAmc
                                  ? m.matchedAmc.toUpperCase()
                                  : "Assign Master AMC"}
                              </span>
                              <ChevronDown
                                size={16}
                                className={`text-slate-400 transition-transform ${isDropdownOpen ? "transform rotate-180" : ""}`}
                              />
                            </div>
                          ...
                          <div className="flex justify-between items-center bg-transparent pt-1">
                            ...
                            <input
                              ...
                              value={m.payoutDay}
                              onChange={(e) =>
                                handlePayoutDayChange(idx, e.target.value)
                              }
                  ...
                  <div className="hidden lg:flex flex-col flex-1 overflow-hidden">
                    <div className="grid grid-cols-12 gap-3 items-center px-8 py-3.5 text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest bg-slate-100 dark:bg-[#0B1120] border-b border-slate-200 dark:border-slate-800 shrink-0 shadow-sm z-20">
                      ...
                      <div className="col-span-2 text-right pr-4 text-blue-600/70 dark:text-indigo-400/70">
                        Base Commission
                      </div>
                      <div className="col-span-1 text-right pr-4 text-purple-600/70 dark:text-purple-400/70">
                        GST
                      </div>
                      <div className="col-span-2 text-right pr-4 text-emerald-600 dark:text-emerald-500">
                        Total Amount
                      </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="col-span-3 flex flex-col justify-center pl-3 pr-2 py-2 overflow-hidden">
                              <span
                                className="uppercase tracking-tight font-black text-[12px] text-slate-800 dark:text-white truncate"
                                title={m.ledgerName}
                              >
                                {m.ledgerName}
                              </span>
                            ...
                            >
                              ...
                              >
                                ...
                                <ChevronDown
                                  size={16}
                                  className={`shrink-0 text-slate-400 transition-transform duration-200 ${isDropdownOpen ? "transform rotate-180 text-blue-500 dark:text-indigo-400" : ""}`}
                                />
                              </div>
              ...
              <div className="px-5 lg:px-8 py-5 bg-white dark:bg-[#0F172A] border-t border-slate-200 dark:border-slate-800 flex flex-col lg:flex-row justify-between lg:items-center gap-5 lg:gap-0 shrink-0 z-10">
                <div className="flex items-center justify-center lg:justify-start gap-3 text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest select-none text-center lg:text-left">
                  <div className="w-8 h-8 rounded-full bg-blue-50 dark:bg-indigo-500/10 flex items-center justify-center">
                    <ShieldCheck
                      size={16}
                      className="text-blue-600 dark:text-indigo-400 shrink-0"
                    />
                  </div>
