...
const ResultStep = ({
  ...
  const activeArnObject = useMemo(
    () =>
      (arns || []).find(
        (a) =>
          String(a._id) === String(arnId) ||
          String(a.arnCode) === String(arnId),
      ),
    [arns, arnId],
  );
  ...
  useEffect(() => {
    ...
    Object.entries(bankMap).forEach(([bank, txs]) => {
      ...
      txs.forEach((tx) => {
        ...
        let groupKey = isManual
          ? `MANUAL_${manualCounter++}`
          : `${tx.type}_${partyLedger}`;
      ...
      groups.forEach((group) => {
        ...
        if (group.type === "RECEIPT") {
          ...
          const salesTxs = group.transactions.filter(
            (t) => isTrue(t.isSales) && isTrue(t.isSalesApproved),
          );
          salesTxs.forEach((tx) => {
            ...
            let activeSalesLedger =
              ...
              salesIncomeLedger ||
              masterLedgers.find((l) =>
                l.name?.toLowerCase().includes("mf com"),
              )?.name ||
      ...
      finalUiGroups[bank] = groups.sort((a, b) => {
        ...
        const aHasSales = a.vouchersList.some(
          (vId) =>
            generatedVouchers.find((v) => v.id === vId)?.vType === "SALES",
        );
        const bHasSales = b.vouchersList.some(
          (vId) =>
            generatedVouchers.find((v) => v.id === vId)?.vType === "SALES",
        );
  ...
  const unpushedCount = useMemo(() => {
    return vouchers.filter(
      (v) => v.vType !== "MANUAL" && v.status !== "SUCCESS",
    ).length;
  }, [vouchers]);
  ...
  const processBatch = async (idsToProcess) => {
    ...
    for (let i = 0; i < idsToProcess.length; i++) {
      ...
      setVouchers((prev) =>
        prev.map((v) =>
          v.id === vId ? { ...v, status: "PROCESSING", errorMsg: null } : v,
        ),
      );
      ...
      try {
        const dStr =
          targetVoucher.vType === "SALES"
            ? tx.invoiceBillingDate || tx.date
            : tx.date;
      ...
      if (targetVoucher.vType === "SALES") {
        ...
        const ledgerObj = masterLedgers.find(
          (l) => (l.name || "").toUpperCase().trim() === normalizedTargetLedger,
        );
        ...
        const salesData = {
          ...
          date: safeDate,
          invoiceNumber:
            tx.invoiceNumber || `INV-${tx._id.slice(-5).toUpperCase()}`,
          ledgerName: finalPartyLedger,
          incomeLedger: finalSalesIncomeLedger,
          amount: targetVoucher.baseAmount,
          gstType:
            targetVoucher.cgst > 0 || targetVoucher.sgst > 0
              ? "LOCAL"
              : targetVoucher.igst > 0
                ? "INTERSTATE"
                : "NONE",
          cgstLedger: "CGST",
          sgstLedger: "SGST",
          igstLedger: "IGST",
          cgstAmount: targetVoucher.cgst,
          sgstAmount: targetVoucher.sgst,
          igstAmount: targetVoucher.igst,
          narration: finalNarration,
          partyState: ledgerObj?.stateName || "",
          partyCountry: ledgerObj?.country || "India",
          partyGstRegType:
            ledgerObj?.gstRegistrationType ||
            (ledgerObj?.gstin ? "Regular" : "Unregistered"),
      ...
      try {
        const response = await request("/tally/proxy", "POST", {
          xml: xmlPayload,
        });
        const responseStr =
        ...
        if (
          responseStr.includes("<CREATED>1</CREATED>") ||
          responseStr.includes("CREATED: 1")
        ) {
          isSuccess = true;
        } else {
          errorMsg = responseStr.includes("Line Error")
            ? "Missing Master Ledger in Tally."
            : "Rejected by Tally.";
        }
      } catch (err) {
    ...
    setVouchers((current) => {
      const remainingPending = current.filter(
        (v) => v.status === "PENDING" && v.vType !== "MANUAL",
      ).length;
      const totalFailedNow = current.filter(
        (v) => v.status === "FAILED",
      ).length;
      ...
      if (
        ...
        toast.error(
          `${totalFailedNow} voucher(s) failed to post. See alert banner.`,
        );
      }
      return current;
    });
  ...
  const handleSyncType = (type) =>
    processBatch(
      vouchers
        .filter((v) => v.status === "PENDING" && v.vType === type)
        .map((v) => v.id),
    );
  ...
  const handleSyncAll = () =>
    processBatch(
      vouchers
        .filter((v) => v.status === "PENDING" && v.vType !== "MANUAL")
        .map((v) => v.id),
    );
  const handleRetryFailed = () =>
    processBatch(
      vouchers.filter((v) => v.status === "FAILED").map((v) => v.id),
    );
  ...
  const getGroupTheme = (type) => {
    if (type === "RECEIPT")
      return {
        ...
        icon: <ArrowRightLeft size={16} strokeWidth={2.5} />,
      };
    if (type === "PAYMENT")
      return {
            ...
            <div className="flex items-start gap-3.5">
              ...
              <div className="flex flex-col">
                ...
                <p className="text-xs text-rose-800/80 dark:text-rose-300/80 mt-0.5 max-w-2xl">
                  {failedVouchers[0]?.errorMsg || "Tally rejected the entry."}{" "}
                  Verify ledger master entries in Tally.
                </p>
              </div>
            </div>
        ...
        <div className="bg-white dark:bg-[#0E131F] rounded-md border-2 border-slate-200 dark:border-white/10 p-4 shadow-sm flex flex-col gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs font-mono">
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/10">
                <span className="flex items-center gap-1.5 text-blue-600 dark:text-blue-400 font-bold">
                  ...
                  <span className="text-[10px] opacity-70 font-normal">
                    ({stats.salesPending} P | {stats.salesSuccess} S)
                  </span>
                </span>
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/10">
                ...
                <button
                  ...
                  disabled={
                    isProcessing ||
                    (stats.receiptsPending === 0 && stats.paymentsPending === 0)
                  }
                  onClick={() =>
                    handleMarkCategoryAsPushed(["RECEIPT", "PAYMENT"])
                  }
                ...
                <button
                  ...
                  disabled={
                    isProcessing ||
                    (stats.receiptsSuccess === 0 && stats.paymentsSuccess === 0)
                  }
                  onClick={() =>
                    handleUnmarkCategoryAsPushed(["RECEIPT", "PAYMENT"])
                  }
                  className="cursor-pointer text-[10px] font-bold text-slate-600 dark:text-slate-400 hover:text-amber-600 dark:hover:text-amber-400 uppercase tracking-tight disabled:opacity-30 disabled:cursor-not-allowed"
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                <Banknote size={13} /> Push Banking (
                {stats.receiptsPending + stats.paymentsPending})
              </button>
            ...
            <div className="flex flex-wrap items-center gap-2 border-t lg:border-t-0 border-slate-100 dark:border-white/5 pt-2 lg:pt-0">
              ...
              {stats.success > 0 && (
                ...
                >
                  <Undo2 size={13} strokeWidth={2.5} /> Unmark All (
                  {stats.success})
                </button>
              )}
              ...
              >
                <CheckCheck size={13} strokeWidth={2.5} /> Mark All Pushed (
                {unpushedCount})
              </button>
            </div>
          </div>
        ...
        {Object.keys(uiGroups).length > 0 && (
          <div className="flex border-b border-slate-300 dark:border-white/10 mb-6 overflow-x-auto no-scrollbar gap-2">
            {Object.keys(uiGroups).map((bank) => {
              ...
              const bankPending = uiGroups[bank].reduce(
                (acc, g) =>
                  acc +
                  g.vouchersList.filter(
                    (vId) =>
                      vouchers.find((x) => x.id === vId)?.status ===
                        "PENDING" &&
                      vouchers.find((x) => x.id === vId)?.vType !== "MANUAL",
                  ).length,
                0,
              );
              const bankFailed = uiGroups[bank].reduce(
                (acc, g) =>
                  acc +
                  g.vouchersList.filter(
                    (vId) =>
                      vouchers.find((x) => x.id === vId)?.status === "FAILED",
                  ).length,
                0,
              );
        ...
        {activeBankTab && uiGroups[activeBankTab] && (
          <div className="flex flex-col gap-8">
            {uiGroups[activeBankTab].map((group, gIdx) => {
              ...
              const groupVouchers = group.vouchersList
                .map((vId) => vouchers.find((v) => v.id === vId))
                .filter(Boolean);
              const groupPending = groupVouchers.filter(
                (v) => v.status === "PENDING" && v.vType !== "MANUAL",
              ).length;
              const groupSuccess = groupVouchers.filter(
                (v) => v.status === "SUCCESS" && v.vType !== "MANUAL",
              ).length;
              ...
              return (
                <div key={group.id || gIdx} className="w-full">
                  ...
                  <div
                    className={`flex flex-col sm:flex-row sm:items-end justify-between gap-3 pb-2 mb-3 ${theme.border}`}
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <span
                        className={`px-2 py-0.5 rounded-sm border text-[10px] font-mono font-bold uppercase tracking-wider flex items-center gap-1 ${theme.badge}`}
                      >
                        {theme.icon}
                        {group.type}
                      </span>
                    ...
                    <div className="flex items-center gap-3">
                      <span
                        className={`font-mono text-base font-black mr-2 ${theme.text}`}
                      >
                        {group.type === "RECEIPT"
                          ? "+"
                          : group.type === "PAYMENT"
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          {groupSuccess > 0 && (
                            <button
                              ...
                              disabled={isProcessing}
                              onClick={() =>
                                handleUnmarkGroupAsPushed(group.id)
                              }
                          ...
                          >
                            <Play size={11} className="fill-current" /> Sync (
                            {groupPending})
                          </button>
                        </div>
                      )}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-[1fr_36px_1fr] w-full gap-4 items-start">
                    ...
                    <div className="space-y-1.5">
                      ...
                      {group.transactions.map((tx, tIdx) => (
                        <div
                          key={tIdx}
                          className="p-2.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-white/5 rounded-sm flex justify-between items-start gap-3"
                        >
                    ...
                    <div className="space-y-2 border-t lg:border-t-0 border-slate-200 dark:border-white/10 pt-3 lg:pt-0">
                      ...
                      {groupVouchers.map((v) => {
                        ...
                        if (v.vType === "SALES") {
                          ...
                          const ledgerObj = masterLedgers.find(
                            (l) =>
                              (l.name || "").toUpperCase().trim() ===
                              resolvedPartyLedger.toUpperCase().trim(),
                          );
                          const isFailed = v.status === "FAILED";
                          return (
                            <div
                              key={v.id}
                              className={`bg-white dark:bg-[#0E131F] border ${isFailed ? "border-rose-400 dark:border-rose-500/50 border-l-4 border-l-rose-500" : "border-slate-200 dark:border-white/10 border-l-3 border-l-blue-500"} rounded-sm overflow-hidden shadow-xs`}
                            >
                                ...
                                <div className="flex flex-col gap-0.5 min-w-0">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-mono text-slate-400">
                                      {v.rawTx.invoiceNumber ||
                                        `INV-${v.rawTx._id.slice(-5).toUpperCase()}`}
                                    </span>
                                  </div>
                              ...
                              <VoucherStatusFooter
                                ...
                                onMarkPushed={() =>
                                  handleMarkSingleAsPushed(v.id)
                                }
                                onUnmarkPushed={() =>
                                  handleUnmarkSingleAsPushed(v.id)
                                }
                        ...
                        if (v.vType === "RECEIPT" || v.vType === "PAYMENT") {
                          ...
                          return (
                            <div
                              key={v.id}
                              className={`bg-white dark:bg-[#0E131F] border ${isFailed ? "border-rose-400 dark:border-rose-500/50" : "border-slate-200 dark:border-white/10"} border-l-4 ${config.border} rounded-sm overflow-hidden shadow-xs`}
                            >
                              <div className="p-3 flex justify-between items-center gap-3">
                                <div className="flex flex-col min-w-0">
                                  <span
                                    className={`text-[9px] font-mono font-bold uppercase tracking-widest ${config.text}`}
                                  >
                                    {config.label}
                                  </span>
                                ...
                                <span
                                  className={`font-mono text-xs font-black ${config.text} shrink-0`}
                                >
                                  {formatINR(v.amount)}
                                </span>
                              </div>
                        ...
                        if (v.vType === "MANUAL") {
                          return (
                            <div
                              key={v.id}
                              className="p-3 rounded-sm border border-slate-200 dark:border-white/10 border-l-3 border-l-amber-500 bg-white dark:bg-slate-900/60 flex justify-between items-center gap-3"
                            >
                              <div className="flex flex-col min-w-0">
      ...
      <div className="flex items-center">
        ...
        {v.status === "SUCCESS" && (
          <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 text-[10px] font-mono font-bold uppercase tracking-wider">
            <CheckCircle2 size={13} strokeWidth={2.5} /> Synced{" "}
            {v.voucherNo && `(${v.voucherNo})`}
          </span>
        )}
        {v.status === "FAILED" && (
          <div className="flex items-center gap-1 text-rose-600 dark:text-rose-400 text-[10px] font-mono font-bold uppercase tracking-wider cursor-help relative group">
            <AlertCircle size={13} strokeWidth={2.5} /> Failed:{" "}
            {v.errorMsg || "Rejected"}
