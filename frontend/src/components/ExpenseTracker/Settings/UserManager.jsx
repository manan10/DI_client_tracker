...
const UserManager = () => {
  ...
  const initialForm = {
    ...
    phone: "",
    password: "",
    allowedApps: ["CLIENT_TRACKER"], // Default access
  };
  ...
  useEffect(() => {
    let isMounted = true;
    // Decoupled mount to avoid cascading render warnings
  ...
  const handleDeleteUser = async (id, name) => {
    if (window.confirm(`Are you sure you want to remove ${name} from the family hub?`)) {
      try {
    ...
    <div className="space-y-10 animate-in fade-in duration-500 text-left">
      ...
      <div className="flex justify-between items-end border-b border-slate-100 dark:border-slate-800 pb-8">
        <div className="text-left">
          <h3 className="text-3xl font-[1000] text-slate-900 dark:text-white uppercase tracking-tighter italic leading-none pt-1 text-left">
            Members
          </h3>
          <p className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.3em] mt-3 italic leading-none pt-1 text-left">
        ...
        {users.map((u) => (
          ...
          >
            <div className="flex items-center gap-5 text-left">
              <div className="w-14 h-14 rounded-2xl bg-white dark:bg-slate-800 flex items-center justify-center text-slate-400 border border-slate-100 dark:border-slate-700 shadow-sm shrink-0 transition-colors group-hover:text-emerald-500">
                <User size={24} />
              </div>
            ...
            <div className="flex gap-2 shrink-0">
              ...
              <button onClick={() => handleDeleteUser(u._id, u.name)} className="p-3.5 bg-white dark:bg-slate-800 text-slate-400 rounded-xl border border-slate-200 dark:border-slate-700 hover:text-red-500 transition-colors shadow-sm">
                <Trash2 size={18} />
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center px-0">
          ...
          >
            ...
            <div className="flex justify-between items-start mb-10">
              ...
              <button type="button" onClick={() => setIsModalOpen(false)} className="p-3.5 bg-slate-50 dark:bg-slate-800 text-slate-400 rounded-full">
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input type="email" required placeholder="email@domain.com" className="w-full bg-slate-50 dark:bg-slate-900 p-6 pl-14 rounded-3xl text-xs font-black text-slate-900 dark:text-white outline-none border border-slate-100 dark:border-slate-800 focus:border-emerald-500/30 transition-all" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} />
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Secure Phone</label>
                  <div className="relative">
              ...
              <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Tool Access Control</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    { id: 'CLIENT_TRACKER', label: 'Client Tracker', desc: 'Investment & AUM', icon: Monitor },
                    { id: 'EXPENSE_TRACKER', label: 'Expense App', desc: 'Family Spends', icon: Wallet }
                  ].map(app => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative flex items-center gap-4 p-5 rounded-[2.2rem] border transition-all text-left group ${
              ...
              >
                {loading ? <Loader2 className="animate-spin" size={20} /> : <><UserCheck size={20} strokeWidth={3} /><span>{updatingUser ? "Confirm Profile" : "Register Member"}</span></>}
              </button>
            </div>
          </form>
        </div>
      ...
      {editingUser && (
        <div className="fixed inset-0 z-50 flex items-end justify-center px-0">
          ...
          >
            ...
            <div className="flex justify-between items-start mb-10">
              <div className="space-y-1.5">
                <h2 className="text-3xl font-[1000] text-slate-900 dark:text-white uppercase tracking-tighter italic leading-none pt-1">New Key</h2>
            ...
            <div className="space-y-8">
              ...
              <button type="submit" disabled={loading} className="w-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 h-20 rounded-[2.5rem] font-[1000] uppercase text-xs tracking-[0.5em] shadow-2xl active:scale-98 flex items-center justify-center gap-3">
                {loading ? <Loader2 className="animate-spin" /> : <Shield size={20} />}Confirm Reset
              </button>
            </div>
          </form>
        </div>
