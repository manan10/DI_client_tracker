      ...
      <div className="bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-white/10 rounded-lg shadow-sm overflow-hidden flex flex-col min-h-125">
        ...
        {processedData.length === 0 && (
          <div className="px-6 py-20 text-center flex flex-col items-center justify-center opacity-40 flex-1">
            <Landmark size={40} strokeWidth={1.5} className="mb-4 text-slate-500" />
            <p className="text-[11px] font-[1000] uppercase tracking-[0.3em] text-slate-500">Registry List Clear</p>
          </div>
        )}
        ...
        {processedData.length > 0 && (
          <div className="bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-white/10 p-4 flex items-center justify-between mt-auto">
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
              Showing <span className="text-slate-900 dark:text-white font-[1000]">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="text-slate-900 dark:text-white font-[1000]">{Math.min(currentPage * itemsPerPage, processedData.length)}</span> of <span className="text-slate-900 dark:text-white font-[1000]">{processedData.length}</span>
            </p>
