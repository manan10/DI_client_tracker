...
const AmcManagement = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 relative">
        ...
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-4xl p-8 border border-slate-200 dark:border-slate-800 shadow-2xl scale-in-center">
            ...
            <div className="flex gap-3 mt-8">
              ...
              <button onClick={executeDelete} className="flex-1 py-4 bg-rose-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-rose-700 shadow-lg shadow-rose-600/20 transition-all">Delete</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-3 bg-white dark:bg-slate-950 p-2 rounded-2xl border-2 border-slate-100 dark:border-slate-800 focus-within:border-emerald-500/50 transition-all">
        <input 
          value={newAmc}
        ...
        />
        <button onClick={handleAdd} className="bg-emerald-600 text-white px-8 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-700 transition-colors">Add AMC</button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {amcs.map((amc) => (
          <div key={amc._id} className="flex items-center justify-between p-5 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-transparent hover:border-emerald-500/20 transition-all">
            <div className="flex items-center gap-4 flex-1">
              <Landmark size={20} className={editingId === amc._id ? "text-emerald-500" : "text-slate-400"} />
              {editingId === amc._id ? (
                <input autoFocus value={editValue} onChange={(e) => setEditValue(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleUpdate(amc._id)} className="bg-white dark:bg-slate-800 border-2 border-emerald-500 rounded-xl px-3 py-1.5 text-sm font-black w-full outline-none dark:text-white" />
              ) : (
            ...
            <div className="flex gap-1 ml-4">
              {editingId === amc._id ? (
                <button onClick={() => handleUpdate(amc._id)} className="p-2 text-emerald-600 bg-white dark:bg-slate-800 rounded-lg shadow-sm"><Check size={18}/></button>
              ) : (
                <button onClick={() => {setEditingId(amc._id); setEditValue(amc.name)}} className="p-2 text-slate-400 hover:text-amber-500 hover:bg-amber-100/50 dark:hover:bg-amber-900/20 rounded-lg transition-all"><Edit2 size={18}/></button>
              )}
              <button onClick={() => setDeleteConfirm({id: amc._id, name: amc.name})} className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-100/50 dark:hover:bg-rose-900/20 rounded-lg transition-all"><Trash2 size={18}/></button>
            </div>
          </div>
        ))}
      </div>
