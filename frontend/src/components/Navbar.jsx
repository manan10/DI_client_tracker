...
import { 
  ...
  SettingsIcon, Menu, X, Files, BarChart3, Lock, Grid, ListTodo, TestTubeIcon, Sparkles
} from 'lucide-react';
        ...
        <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 2xl:px-16">
          <div className="relative flex justify-between items-center h-18">
            ...
            <div className="flex items-center shrink-0 min-w-0 z-20">
              <Link to="/dashboard" className="flex items-center gap-3.5 group outline-none">
                <div className="p-1 rounded-xl bg-white/5 border border-white/10 group-hover:border-emerald-500/40 transition-all duration-300 shadow-2xs">
                  <img src={Logo} alt="Logo" className="h-8 sm:h-8.5 w-auto group-hover:scale-105 transition-transform duration-300" />
                </div>
            ...
            <div className="flex items-center gap-2.5 sm:gap-3 shrink-0 z-20">
              ...
              <div className="hidden sm:flex items-center gap-3 pl-1.5 p-1.5 pr-3.5 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 transition-all cursor-default shadow-xs">
                ...
                <div className="flex flex-col text-left min-w-0 pr-1">
                  ...
                  <span className={`text-[9px] font-mono font-bold tracking-wider mt-1 leading-none uppercase ${user?.isAdmin ? 'text-emerald-400' : 'text-slate-400'}`}>
          ...
          {mainMobileItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <div className={`mt-0.5 transition-colors duration-200 ${isActive ? 'text-emerald-400' : 'text-slate-400 group-hover:text-white'}`}>
                  <Icon size={20} strokeWidth={isActive ? 2.5 : 2} />
                </div>
                <span className={`text-[10px] font-bold tracking-wide uppercase ${isActive ? 'text-emerald-400' : 'text-slate-400 group-hover:text-white'}`}>
                  {item.name}
                </span>
              </Link>
            );
