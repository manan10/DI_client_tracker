...
import { Search, History, Calendar, RefreshCw, AlertCircle, FileSpreadsheet } from 'lucide-react';
  ...
  if (isLoading && history.length === 0) {
    return (
      <div className="space-y-3 mt-4">
        ...
        {[1, 2, 3].map(i => (
          <div key={i} className="animate-pulse h-14 bg-white dark:bg-[#0B1120] rounded-xl border border-slate-200/80 dark:border-white/10" />
        ))}
      </div>
    );
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] font-mono text-slate-400 dark:text-slate-500 mt-0.5">
              {filteredHistory.length} Reconciled Statement{filteredHistory.length === 1 ? '' : 's'}
            </p>
          </div>
        </div>
