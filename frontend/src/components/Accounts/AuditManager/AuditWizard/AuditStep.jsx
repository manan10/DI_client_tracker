...
const AuditStep = ({ selection, setSelection, masterLedgers }) => {
  ...
  useEffect(() => {
    const firstUnverified = displayTransactions.findIndex(t => !(selection.verifiedIds || []).includes(t._id));
  ...
  const isCurrentVerified = activeTx ? (selection.verifiedIds || []).includes(activeTx._id) : false;
  ...
  const grandTotals = useMemo(() => {
    ...
    const receiptTotal = receipts.reduce((acc, t) => acc + Math.abs(t.amount || 0), 0);
    const paymentTotal = payments.reduce((acc, t) => acc + Math.abs(t.amount || 0), 0);

    const verifiedReceipts = receipts.filter(t => (selection.verifiedIds || []).includes(t._id)).length;
    const verifiedPayments = payments.filter(t => (selection.verifiedIds || []).includes(t._id)).length;
  ...
  const bankDirectory = useMemo(() => {
    return availableBanks.map(bank => {
      const bankTxs = transactions.filter(t => t.bank === bank || (!t.bank && availableBanks[0] === bank));
      ...
      const totalReceiptAmount = receipts.reduce((sum, t) => sum + Math.abs(t.amount), 0);
      const totalPaymentAmount = payments.reduce((sum, t) => sum + Math.abs(t.amount), 0);
  ...
  const handleUpdate = useCallback(async (txId, payload, silent = false) => {
  ...
  const handleVerifyAndNext = useCallback(async () => {
    ...
    try {
      ...
      const nextUnverifiedIdx = displayTransactions.findIndex((t, idx) => idx > currentIndex && !(selection.verifiedIds || []).includes(t._id));
      
      if (nextUnverifiedIdx !== -1) {
        setCurrentIndex(nextUnverifiedIdx);
      } else {
        const wrapIdx = displayTransactions.findIndex(t => !(selection.verifiedIds || []).includes(t._id));
        if (wrapIdx !== -1 && wrapIdx !== currentIndex) {
          setCurrentIndex(wrapIdx);
        } else if (currentIndex < displayTransactions.length - 1) {
          setCurrentIndex(prev => prev + 1);
        } else {
          toast.success(`All ${activeTab.toLowerCase()}s for ${currentBank} verified!`);
        }
      }
    } finally {
      setIsVerifying(false);
    }
  }, [activeTx, isVerifying, handleUpdate, displayTransactions, currentIndex, selection.verifiedIds, activeTab, currentBank]);
  ...
  const handleJumpNextPending = useCallback(() => {
    const nextUnverifiedIdx = displayTransactions.findIndex(
      (t, idx) => idx > currentIndex && !(selection.verifiedIds || []).includes(t._id)
    );
    if (nextUnverifiedIdx !== -1) {
      setCurrentIndex(nextUnverifiedIdx);
    } else {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.key === 'Escape') {
        if (ledgerModalOpen) { setLedgerModalOpen(false); setLedgerSearch(""); return; }
      ...
      if (e.key === 'ArrowRight' || e.key.toLowerCase() === 'n') {
        e.preventDefault();
        setCurrentIndex(prev => Math.min(prev + 1, displayTransactions.length - 1));
      }
      ...
      if (e.key.toLowerCase() === 'm' && activeTx) {
        e.preventDefault();
        handleUpdate(activeTx._id, { isMarkedForManualEntry: !activeTx.isMarkedForManualEntry });
      }

      if (e.key === '1') { e.preventDefault(); setActiveTab('RECEIPT'); }
      if (e.key === '2') { e.preventDefault(); setActiveTab('PAYMENT'); }
      if (e.key === '?') { e.preventDefault(); setShowShortcuts(p => !p); }
    };

    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [ledgerModalOpen, showShortcuts, activeTx, handleVerifyAndNext, handleUpdate, toggleComm, displayTransactions.length]);

  const pendingCount = displayTransactions.filter(t => !(selection.verifiedIds || []).includes(t._id)).length;
      ...
      <div className="flex flex-col lg:flex-row h-full w-full overflow-hidden absolute inset-0 bg-white dark:bg-[#06080E] font-sans text-left min-w-0">
        ...
        <section className="flex-1 flex flex-col justify-between min-w-0 overflow-hidden bg-slate-100/90 dark:bg-[#07090E] h-full border-l border-slate-200/80 dark:border-white/10 shadow-inner">
          <TransactionInspector
            ...
            onToggleSale={() => handleUpdate(activeTx._id, { isSales: !activeTx.isSales })}
            onToggleComm={toggleComm}
            onToggleManual={() => handleUpdate(activeTx._id, { isMarkedForManualEntry: !activeTx.isMarkedForManualEntry })}
            onCustomNarrationChange={(val) => handleUpdate(activeTx._id, { customNarration: val }, true)}
            onPrev={() => setCurrentIndex(prev => Math.max(prev - 1, 0))}
            onNext={() => setCurrentIndex(prev => Math.min(prev + 1, displayTransactions.length - 1))}
        ...
        <LedgerSearchModal 
          ...
          onClose={() => { setLedgerModalOpen(false); setLedgerSearch(""); }}
        />
