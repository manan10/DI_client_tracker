...
const CommissionMapperModal = ({ isOpen, onClose, selection, commissionLines, formatINR, onSuccess }) => {
  ...
  // FIXED: Moved to top scope so the submit function can safely read it for the text progress output
  ...
  const handleCommitPayload = async () => {
    ...
    try {
      ...
      // Inside handleCommitPayload function block in CommissionMapperModal.jsx:
    if (response?.success) {
        await new Promise(r => setTimeout(r, 500));
        ...
        if (onSuccess) onSuccess(); // Hydrates active parent state lock
  ...
  return (
    <div className="fixed inset-0 z-200 flex items-center justify-center p-6 bg-slate-950/40 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="bg-white dark:bg-[#0B0C10] w-full max-w-5xl rounded-xl border border-slate-200 dark:border-white/5 shadow-2xl flex flex-col h-[70vh] max-h-[70vh] overflow-hidden text-[12px]">
        
        {/* HEADER */}
        <div className="px-8 py-4 border-b border-slate-100 dark:border-white/5 flex justify-between items-center bg-slate-50 dark:bg-black/10 shrink-0">
          ...
          {!isSaving && (
            <button onClick={onClose} className="p-1.5 text-slate-400 hover:text-rose-500 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 transition-all"><X size={16}/></button>
          )}
        </div>

        {/* CONDITION A: ACCOUNTING PROGRESS PANEL */}
        {isSaving ? (
          <div className="flex-1 bg-white dark:bg-[#0B0C10] p-8 flex flex-col justify-between overflow-hidden select-none">
            <div className="space-y-6 max-w-2xl mx-auto w-full pt-6">
              ...
              {/* STATUS LOG DETAILS */}
              <div className="space-y-3.5 pl-8 text-left text-[12px] font-bold text-slate-600 dark:text-slate-300">
            ...
            <div className="max-w-2xl mx-auto w-full mt-8 space-y-3 shrink-0 pb-6">
              <div className="flex justify-between items-end font-black text-[10px] text-slate-400 uppercase tracking-widest">
              ...
              {executionComplete && (
                <div className="pt-6 flex justify-center animate-in fade-in zoom-in-95 duration-200">
                  <button
                    onClick={onClose}
                    className="bg-slate-900 hover:bg-slate-800 text-white font-black text-[10px] uppercase tracking-widest px-8 py-3 rounded-lg shadow-md transition-transform active:scale-95"
                  >
                    Close Assistant
                  </button>
                </div>
          ...
          <>
            <div className="flex-1 overflow-y-auto px-8 py-4 no-scrollbar min-h-0 relative bg-white dark:bg-[#0B0C10]">
              {loadingAmcs ? (
                <div className="h-full flex flex-col items-center justify-center gap-3 opacity-50">
                  <Loader2 className="animate-spin text-emerald-600" size={24} />
                  <p className="text-[10px] font-black uppercase tracking-widest">Querying live backend AMC parameters...</p>
                </div>
              ) : (
            ...
            {/* LOWER FOOTER */}
            <div className="px-8 py-4 bg-slate-50 dark:bg-black/10 border-t border-slate-100 dark:border-white/5 flex justify-between items-center shrink-0">
              <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider select-none">
                <ShieldCheck size={14} className="text-emerald-600"/> Batch balancing verified. Ready to synchronize advisory dashboards.
              </div>
              <div className="flex gap-3">
                <button
                  onClick={onClose}
                  className="px-5 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex items-center gap-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-black text-[10px] uppercase tracking-widest px-6 py-3 rounded-xl shadow-lg transition-all active:scale-95 disabled:opacity-40 border border-emerald-700/20"
                >
                  Confirm and Log Commissions <ArrowRight size={14} strokeWidth={3}/>
                </button>
              </div>
            </div>
          </>
