...
const Settings = () => {
  ...
  const tabs = [
    ...
    { id: 'tally', label: 'Tally', fullLabel: 'Import Ledgers', icon: Terminal, locked: false },
    ...
    { id: 'data', label: 'Sync', fullLabel: 'WE Sync', icon: History, locked: false },
    { id: 'compliance', label: 'Compliance', fullLabel: 'Compliance', icon: ShieldCheck, locked: true },
  ];
    ...
    return (
      <div className="h-screen w-full flex items-center justify-center bg-white dark:bg-[#08090A]">
        <div className="flex flex-col items-center gap-6">
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 animate-pulse">Synchronizing Core</span>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#FDFDFD] dark:bg-[#08090A] transition-colors duration-500 pb-24 lg:pb-0 font-sans">
      ...
      <main className="max-w-[98%] mx-auto px-4 sm:px-8 lg:py-16 py-8">
        ...
        <div className="lg:hidden mb-10 border-l-8 border-emerald-500 pl-6">
          <h2 className="text-4xl font-black text-slate-950 dark:text-white uppercase tracking-tighter italic leading-none">Settings</h2>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mt-2">Control & Configuration</p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-start">
          ...
          <aside className="w-full lg:w-80 lg:sticky lg:top-32 z-40">
            <div className="hidden lg:block mb-12 border-l-8 border-slate-950 dark:border-emerald-500 pl-8">
              <h2 className="text-5xl font-black text-slate-950 dark:text-white uppercase tracking-tighter italic leading-none">Settings</h2>
              <p className="text-[11px] font-black text-emerald-600 uppercase tracking-[0.4em] mt-3">Portal Engine</p>
            </div>
            ...
            <nav className="grid grid-cols-2 md:grid-cols-4 lg:flex lg:flex-col gap-3 w-full">
              {tabs.map((tab) => {
                ...
                return (
                  ...
                  >
                    <tab.icon size={18} strokeWidth={isActive ? 3 : 2} className="shrink-0 transition-transform group-hover:scale-110" />
                    <span className="truncate">{tab.fullLabel}</span>
                    
                    {!tab.locked && isActive && (
                        <ChevronRight size={14} className="ml-auto animate-in slide-in-from-left-2" />
                    )}
          ...
          <div className="flex-1 w-full bg-white dark:bg-[#0E1012] border border-slate-100 dark:border-white/5 shadow-[20px_20px_60px_rgba(0,0,0,0.02)] dark:shadow-none relative min-h-[70vh] flex flex-col overflow-hidden">
            <div className="flex-1 p-6 md:p-12 lg:p-20">
              {renderContent()}
            </div>
