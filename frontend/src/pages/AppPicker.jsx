...
const allApps = [
  {
    ...
    theme: {
      ...
      accentBg: 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-200/80 dark:border-emerald-500/20',
      border: 'border-emerald-200/80 dark:border-emerald-500/20 hover:border-emerald-500/60 dark:hover:border-emerald-500/50',
      cardHover: 'hover:shadow-[0_20px_40px_rgba(16,185,129,0.12)] dark:hover:shadow-[0_20px_40px_rgba(16,185,129,0.08)]',
      badgeBg: 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border-emerald-200/80 dark:border-emerald-500/30',
      pillBg: 'bg-emerald-50/80 dark:bg-emerald-500/10 text-emerald-800 dark:text-emerald-300 border-emerald-100 dark:border-emerald-500/20',
  ...
  },
  {
    ...
    theme: {
      ...
      accentBg: 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-200/80 dark:border-indigo-500/20',
      border: 'border-indigo-200/80 dark:border-indigo-500/20 hover:border-indigo-500/60 dark:hover:border-indigo-500/50',
      cardHover: 'hover:shadow-[0_20px_40px_rgba(99,102,241,0.12)] dark:hover:shadow-[0_20px_40px_rgba(99,102,241,0.08)]',
      badgeBg: 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 border-indigo-200/80 dark:border-indigo-500/30',
      pillBg: 'bg-indigo-50/80 dark:bg-indigo-500/10 text-indigo-800 dark:text-indigo-300 border-indigo-100 dark:border-indigo-500/20',
...
const AppPicker = () => {
  ...
  const [isDark, setIsDark] = useState(() => {
    return localStorage.theme === 'dark' || 
      ...
      (!('theme' in localStorage) && !('app-theme' in localStorage) && window.matchMedia('(prefers-color-scheme: dark)').matches);
  });
  ...
  return (
    <div className="h-dvh w-screen bg-[#F8FAFC] dark:bg-[#060A14] text-slate-900 dark:text-slate-100 flex flex-col font-sans overflow-hidden relative selection:bg-emerald-600 selection:text-white transition-colors duration-300">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
        ...
        <div 
          className="absolute -top-[25%] left-1/4 w-[55vw] h-[55vw] max-w-150 max-h-150 rounded-full bg-emerald-400/15 dark:bg-emerald-500/10 blur-[120px] pointer-events-none" 
        />
        <div 
          className="absolute -bottom-[25%] right-1/4 w-[55vw] h-[55vw] max-w-150 max-h-150 rounded-full bg-indigo-400/15 dark:bg-indigo-500/10 blur-[120px] pointer-events-none" 
        />
        {/* Micro-dot SaaS Grid */}
      ...
      <header className="relative z-50 w-full px-4 sm:px-8 py-3 sm:py-4 flex justify-between items-center shrink-0 border-b border-slate-200/70 dark:border-white/10 bg-white/75 dark:bg-[#0B1120]/80 backdrop-blur-xl shadow-[0_2px_15px_rgba(0,0,0,0.02)] dark:shadow-none transition-colors duration-300">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xs sm:text-sm font-[1000] text-slate-900 dark:text-white uppercase tracking-tight leading-none">
                Dalal <span className="text-emerald-600 dark:text-emerald-400">Central</span>
              </h1>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button 
            ...
            title="Sign Out"
          >
            <LogOut size={13} className="transition-transform group-hover:-translate-x-0.5 text-slate-400 dark:text-slate-500 group-hover:text-rose-600 dark:group-hover:text-rose-400" /> 
      ...
      <main className="flex-1 flex flex-col items-center justify-center w-full max-w-6xl mx-auto px-4 sm:px-6 relative z-10 min-h-0 py-2 sm:py-6">
        ...
        <div className="text-center space-y-1 sm:space-y-2 mb-4 sm:mb-8 shrink-0">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/90 dark:bg-slate-900/80 border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-300 text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] shadow-2xs backdrop-blur-md">
            <Sparkles size={12} className="text-emerald-600 dark:text-emerald-400 animate-spin" style={{ animationDuration: '6s' }} />
            <span>Choose App</span>
          </div>
          <h2 className="text-2xl sm:text-4xl font-[1000] text-slate-900 dark:text-white uppercase tracking-tight">
            Select Your <span className="bg-linear-to-r from-emerald-600 via-teal-500 to-indigo-600 dark:from-emerald-400 dark:via-teal-400 dark:to-indigo-400 bg-clip-text text-transparent">Workspace</span>
          </h2>
        ...
        <div className="flex lg:hidden flex-col gap-3 w-full max-w-md mx-auto overflow-y-auto no-scrollbar shrink">
          {authorizedApps.map((app) => (
            ...
            >
              ...
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border ${app.theme.accentBg} group-hover:translate-x-0.5 transition-transform`}>
      ...
      <footer className="relative z-50 w-full px-4 sm:px-8 py-2.5 sm:py-3 flex justify-between items-center shrink-0 border-t border-slate-200/70 dark:border-white/10 bg-white/75 dark:bg-[#0B1120]/80 backdrop-blur-xl text-slate-500 dark:text-slate-400 transition-colors duration-300">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-600 dark:text-slate-300">Secure Encrypted Session</span>
        </div>
        <div className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">
          <span>Logged in as:</span>
          <span className="text-slate-700 dark:text-slate-200 font-black">{user?.name || 'User'}</span>
        </div>
      </footer>
