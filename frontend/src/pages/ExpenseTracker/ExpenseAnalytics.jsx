    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#020617] text-left transition-colors duration-500 pb-24">
      ...
      <main className="w-full max-w-400 mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-12 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white dark:bg-[#0B1120] rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 sm:p-8 flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-8 pb-4 border-b border-slate-100 dark:border-slate-800/80">
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white uppercase tracking-tight">Categorical Spend</h3>
                  <span className="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 rounded-md text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                    {totalCategories > 0 ? `${startIndex + 1}-${Math.min(startIndex + ITEMS_PER_PAGE, totalCategories)} OF ${totalCategories} Categories` : '0 Categories'}
                  </span>
                </div>
              ...
              {/* Pagination Controls */}
              {totalPages > 1 && (
                <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800/80 mt-6">
                  <button 
                    onClick={() => setCategoryPage(p => Math.max(1, p - 1))}
                    disabled={categoryPage === 1}
                    className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 disabled:opacity-30 disabled:hover:bg-transparent transition-colors"
                  >
                    <ChevronLeft size={18} strokeWidth={2.5} />
                  </button>
            ...
            <div className="bg-white dark:bg-[#0B1120] rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 sm:p-8 flex flex-col relative overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 pb-4 border-b border-slate-100 dark:border-slate-800/80 relative z-30">
                <div className="flex items-center gap-3">
                  <h3 className="text-lg font-bold uppercase text-slate-900 dark:text-white tracking-tight">Categorical Trends</h3>
                  ...
                  {isDropdownOpen && (
                    <div className="absolute right-0 top-full mt-2 w-full sm:w-64 bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden py-1.5 animate-in fade-in slide-in-from-top-2 origin-top z-50">
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-4 py-2.5 text-left text-xs font-bold uppercase tracking-wider hover:bg-indigo-50 dark:hover:bg-slate-800 transition-colors ${trendCategory === "All Categories" ? 'text-indigo-600 bg-indigo-50/50 dark:bg-slate-800/50' : 'text-slate-600 dark:text-slate-300'}`}
                      >
                        ALL CATEGORIES
                        {trendCategory === "All Categories" && <Check size={14} className="text-indigo-500" />}
                      </button>
                      ...
                      <div className="max-h-56 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-700">
                        {dropdownOptions.length === 0 ? (
                          ...
                          >
                            <span className="truncate pr-4">{catLabel.toUpperCase()}</span>
