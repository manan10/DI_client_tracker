...
const ExpenseAnalytics = () => {
  ...
  const paginatedCategories = activeCategories.slice(startIndex, startIndex + ITEMS_PER_PAGE);
  ...
  if (loading && !data) return (
    <div className="fixed inset-0 bg-slate-50 dark:bg-[#030712] z-50 flex flex-col items-center justify-center p-6">
      ...
      <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 text-center">Compiling Ledgers</p>
    </div>
  );
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-5 border-b border-slate-200 dark:border-white/10 pb-5">
          <div className="shrink-0 w-full md:w-auto min-w-0 flex flex-col">
            ...
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-[1000] uppercase tracking-tight text-slate-900 dark:text-white leading-none truncate italic">
              Spending <span className="text-emerald-600 dark:text-emerald-500">Analytics</span>
            </h1>
          </div>
            ...
            <div>
              <div className="flex items-center justify-between pb-4 mb-5 border-b border-slate-200 dark:border-white/5 min-w-0">
                ...
                <span className="px-2 py-1 bg-slate-100 dark:bg-slate-800/50 rounded-md text-[9px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest shrink-0 border border-slate-200 dark:border-white/5">
                  {totalCategories > 0 ? `${startIndex + 1}-${Math.min(startIndex + ITEMS_PER_PAGE, totalCategories)} OF ${totalCategories}` : '0'}
                </span>
              </div>
              ...
              {activeCategories.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center bg-slate-50/50 dark:bg-slate-900/30 rounded-xl border border-dashed border-slate-200 dark:border-white/10">
                  <p className="text-xs font-bold uppercase tracking-widest text-slate-400">No categorical data</p>
                </div>
              ) : (
                <div className="space-y-5">
                  {paginatedCategories.map((cat, idx) => {
                    ...
                    const gradient = CHART_COLORS[(startIndex + idx) % CHART_COLORS.length];

                    return (
                      <div key={idx} className="group cursor-pointer min-w-0" onClick={() => setTrendCategory(cat.category)}>
          ...
          <div className="bg-white dark:bg-[#0B1120] rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm p-5 sm:p-6 lg:p-8 flex flex-col relative overflow-hidden min-w-0">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-4 mb-5 border-b border-slate-200 dark:border-white/5 gap-3 relative z-30 min-w-0">
              <div className="flex items-center gap-2 min-w-0">
                ...
                {isTrendLoading && <Activity size={12} className="text-indigo-500 animate-pulse shrink-0" />}
              </div>
              ...
              <div className="relative w-full sm:w-auto min-w-0" ref={dropdownRef}>
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <Filter size={12} className="text-indigo-600 dark:text-indigo-400 shrink-0" />
                ...
                {isDropdownOpen && (
                  <div className="absolute right-0 top-[calc(100%+4px)] w-[calc(100vw-2rem)] sm:w-64 bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-white/10 rounded-lg shadow-xl overflow-hidden py-1.5 animate-in fade-in slide-in-from-top-2 origin-top z-50">
                    <button
                      onClick={() => { setTrendCategory("All Categories"); setIsDropdownOpen(false); }}
                    ...
                    >
                      ALL CATEGORIES
                      {trendCategory === "All Categories" && <Check size={14} className="text-indigo-600 dark:text-indigo-400 shrink-0" />}
                    </button>
                    ...
                    <div className="max-h-48 overflow-y-auto custom-scroll">
                      {dropdownOptions.length === 0 ? (
                         <div className="px-3 py-2 text-[10px] text-slate-400 font-medium italic">No categories found</div>
                        ...
                        >
                          ...
                          {trendCategory === catLabel && <Check size={14} className="text-indigo-600 dark:text-indigo-400 shrink-0" />}
                        </button>
                      ))}
                    </div>
                  </div>
            ...
            {/* Chart Component */}
            <div className={`flex-1 min-h-55 flex items-end justify-between gap-1.5 sm:gap-4 relative pt-6 z-10 transition-opacity duration-300 ${isTrendLoading ? 'opacity-40 blur-sm' : 'opacity-100'}`}>
              
              {/* Background Grid Lines */}
              <div className="absolute inset-0 flex flex-col justify-between pointer-events-none pb-7 pt-6">
                {[...Array(4)].map((_, i) => (
                  <div key={i} className="w-full border-t border-dashed border-slate-200 dark:border-white/5" />
                ))}
              </div>
              ...
              {trendData.map((d, i) => {
                ...
                return (
                  <div key={i} className="flex flex-col items-center w-full group relative z-20 h-full justify-end">
                    ...
                    {/* X-Axis Label */}
                    <span className={`text-[9px] sm:text-[10px] font-bold uppercase tracking-wider mt-2.5 truncate max-w-full px-0.5 ${
                      isCurrentMonth ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400'
                    }`}>
                      {d.label}
                    </span>
                  </div>
