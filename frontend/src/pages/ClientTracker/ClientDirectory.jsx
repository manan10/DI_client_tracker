...
const ClientDirectory = () => {
  ...
  const handleCreateClient = async (e) => {
    ...
    if (familyMode === "existing" && !selectedFamilyClient) {
      setFormError("Please choose an existing client or family head to attach to.");
      return;
    }
    ...
    try {
      ...
      if (res?.success || res?._id || res?.data) {
        ...
        setFormError(res?.message || res?.error || "Failed to register new client.");
      }
    } catch (err) {
      console.error("Client creation failed:", err);
      setFormError(err?.message || "Server error occurred while registering client.");
    } finally {
      setIsSubmitting(false);
    }
  };
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    value={formPan}
                    onChange={(e) => setFormPan(e.target.value.toUpperCase().replace(/\s/g, ""))}
                ...
                <div className="pt-2 border-t border-slate-200 dark:border-white/5 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      <span className="text-xs font-bold uppercase tracking-wide">New Family Group</span>
                      <span className="text-[10px] font-medium opacity-80 mt-1">Make this client the head of a new family portfolio</span>
                    </button>
                    ...
                    >
                      <span className="text-xs font-bold uppercase tracking-wide">Existing Family</span>
                      <span className="text-[10px] font-medium opacity-80 mt-1">Attach as a member to an existing family ledger</span>
                    </button>
                  </div>
                        ...
                        <div className="flex items-center justify-between p-2.5 bg-slate-100 dark:bg-white/5 border border-emerald-500/30 rounded-lg">
                          <div className="flex flex-col">
                            ...
                            <span className="text-[10px] font-mono text-slate-500 dark:text-slate-400">
                              PAN: {selectedFamilyClient.pan} • ID: {selectedFamilyClient.familyId || "Primary"}
                            </span>
                          </div>
                          <button
                            type="button"
