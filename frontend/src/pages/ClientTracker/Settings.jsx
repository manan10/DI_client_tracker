...
const Settings = () => {
  ...
  const tabs = [
    { id: "users", label: "User Management", description: "Manage platform access, roles, and user permissions.", icon: Users, admin: true },
    { id: "amcs", label: "AMC Registry", description: "Configure and map Asset Management Company ledgers.", icon: Landmark },
    { id: "arns", label: "ARN List", description: "Manage mutual fund distribution ARN codes and compliances.", icon: UserCheck },
    { id: "accounts", label: "Bank Accounts", description: "Configure system bank accounts and true-up balances.", icon: Wallet, admin: true },
    { id: "business", label: "AUM Thresholds", description: "Set business logic and categorization thresholds for AUM.", icon: BarChart3 },
    { id: "data", label: "WE Sync", description: "Manage data synchronization with Wealth Elite systems.", icon: History },
    { id: "workflows", label: "Workflow Engine", description: "Configure automated triggers and background processes.", icon: GitBranch },
    { id: "tally", label: "Tally Ledgers", description: "Map and synchronize master ledgers with Tally ERP.", icon: Terminal },
    { id: "system", label: "Appearance", description: "Customize the interface theme and visual preferences.", icon: Bell },
    { id: "device", label: "Biometric Login", description: "Register devices for secure WebAuthn/Biometric access.", icon: Lock },
    { id: "compliance", label: "Compliance", description: "Manage regulatory settings, disclaimers, and EUINs.", icon: ShieldCheck, locked: true },
  ];
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 selection:bg-emerald-500/20 overflow-hidden">
      ...
      <main className="flex-1 flex max-w-400 w-full mx-auto overflow-hidden bg-white dark:bg-[#0B1120] lg:border-x border-slate-200 dark:border-white/5 shadow-sm">
        ...
        <aside className={`w-full lg:w-80 shrink-0 flex flex-col bg-slate-50 dark:bg-slate-900/40 border-r border-slate-200 dark:border-white/5 ${isMobileMenu ? "flex" : "hidden lg:flex"}`}>
          ...
          <div className="p-6 lg:hidden shrink-0 border-b border-transparent dark:border-white/5">
            <h1 className="text-3xl font-black uppercase tracking-tighter italic text-slate-900 dark:text-white">Settings</h1>
            <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1">System Control</p>
          </div>
          ...
          <nav className="flex-1 overflow-y-auto py-4 lg:py-0 space-y-1 no-scrollbar">
            {visibleTabs.map((tab) => {
              ...
              return (
                ...
                >
                  <tab.icon size={18} className={isActive ? "text-emerald-500" : "opacity-60"} />
                  {tab.label}
                  <ChevronRight 
                    size={16} 
        ...
        <section className={`flex-1 flex flex-col h-full bg-white dark:bg-[#0B1120] relative ${isMobileMenu ? "hidden lg:flex" : "flex"}`}>
          ...
          <div className="px-6 py-4 border-b border-slate-200 dark:border-white/5 flex items-center lg:hidden bg-slate-50 dark:bg-slate-900 shrink-0">
              <button onClick={() => setIsMobileMenu(true)} className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-500 hover:text-emerald-600 transition-colors">
          ...
          <div className="hidden lg:flex flex-col px-10 py-8 border-b border-slate-100 dark:border-white/5 shrink-0 bg-white/50 dark:bg-[#0B1120]/50 backdrop-blur-xl z-10">
              <div className="flex items-center gap-3">
                 <h1 className="text-2xl font-black text-slate-300 dark:text-slate-700 uppercase tracking-tighter italic">Settings</h1>
                 <span className="text-slate-200 dark:text-slate-800 font-light text-2xl">/</span>
                 <h2 className="text-xl font-black text-slate-900 dark:text-white flex items-center gap-2 uppercase tracking-tight">
                    <activeTabData.icon className="text-emerald-500" size={22} strokeWidth={2.5} />
                    {activeTabData.label}
                 </h2>
              </div>
