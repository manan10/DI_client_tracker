    ...
    <div className={`h-dvh w-full font-sans select-none overflow-hidden relative transition-colors duration-300 flex flex-col justify-between ${
      ...
      <main className="relative z-10 h-full w-full max-w-7xl 2xl:max-w-375 mx-auto flex flex-col justify-between lg:grid lg:grid-cols-12 lg:gap-14 xl:gap-20 lg:items-center px-4 sm:px-8 lg:px-12 xl:px-16 pt-8 sm:pt-10 lg:py-8 pb-0 sm:pb-6 min-h-0 overflow-y-auto lg:overflow-visible">
        ...
        <div className="w-full lg:col-span-6 flex flex-col items-center lg:items-start text-center lg:text-left shrink-0 my-auto lg:my-0 pb-6 lg:pb-0">
          ...
          <h1 className={`text-3xl sm:text-4xl lg:text-6xl xl:text-7xl 2xl:text-8xl font-[1000] tracking-tight leading-[0.98] uppercase mb-1.5 sm:mb-2 lg:mb-3 ${
        ...
        <div className="w-full lg:col-span-6 max-w-lg lg:max-w-xl xl:max-w-2xl mx-auto shrink-0 mt-auto lg:mt-0">
          <div className={`w-full rounded-t-3xl sm:rounded-2xl lg:rounded-3xl border-t-2 border-x-2 sm:border-2 lg:border-2 transition-all duration-300 shadow-2xl p-6 sm:p-8 lg:p-9 xl:p-11 ${
            isDark 
            ...
            <div className="flex items-center justify-between pb-3.5 sm:pb-4 lg:pb-5 mb-4 sm:mb-5 lg:mb-6 border-b border-slate-200/80 dark:border-slate-800">
              <div className="text-left">
                <h2 className={`text-xl sm:text-2xl lg:text-2xl xl:text-3xl font-[1000] uppercase tracking-tight ${isDark ? 'text-white' : 'text-slate-950'}`}>
                  Sign In
                </h2>
                <p className="text-xs sm:text-sm text-slate-400 font-medium mt-0.5">Choose your account to sign in</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5 lg:space-y-6 text-left w-full">
              ...
              <div className="relative" ref={dropdownRef}>
                  ...
                  {hasBiometrics && (
                    ...
                    >
                      {isBiometricLoading ? (
                        ...
                        <Fingerprint size={15} className="lg:w-4 lg:h-4" strokeWidth={2.4} />
                      )}
                ...
                >
                  <div className="flex items-center gap-3 sm:gap-3.5 lg:gap-4 min-w-0">
                    <div className={`h-10 w-10 sm:h-11 sm:w-11 lg:h-11 lg:w-11 rounded-xl flex items-center justify-center shrink-0 transition-colors ${
                      selectedUser 
                      ...
                      <User size={18} className="lg:w-5 lg:h-5" strokeWidth={2.4} />
                    </div>
                    <div className="flex flex-col items-start truncate">
                      {loadingUsers ? (
                        ...
                        <>
                          <span className={`text-sm sm:text-base font-bold uppercase tracking-tight truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>
                            {selectedUser.name}
                          </span>
                        ...
                        <span className="text-sm sm:text-base font-bold text-slate-400">Select Profile</span>
                      )}
                    </div>
                  </div>
                  <ChevronRight size={18} className={`text-slate-400 lg:w-5 lg:h-5 shrink-0 transition-transform duration-200 ${isOpen ? 'rotate-90 text-emerald-500' : ''}`} />
                </button>

                {/* Dropdown Menu List: In-flow expansion on mobile */}
                {isOpen && (
                  <div className={`mt-2 rounded-xl lg:rounded-2xl border shadow-2xl p-2 max-h-48 sm:max-h-56 lg:max-h-64 overflow-y-auto backdrop-blur-xl ${
                    ...
                    {users.map((u) => (
                      ...
                      >
                        <div className="flex flex-col items-start text-left truncate min-w-0 pr-2">
                          <span className="text-sm font-bold uppercase tracking-tight truncate">{u.name}</span>
                          <span className="text-[10px] sm:text-xs font-mono text-slate-400">@{u.username}</span>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          <div className="flex gap-1.5 text-slate-400 mr-0.5">
                            {u.allowedApps?.includes('CLIENT_TRACKER') && <Monitor size={13} className="lg:w-3.5 lg:h-3.5" title="Client Tracker" />}
                            {u.allowedApps?.includes('EXPENSE_TRACKER') && <Wallet size={13} className="lg:w-3.5 lg:h-3.5" title="Expense Tracker" />}
                          </div>
                          {selectedUser?._id === u._id && <Check size={16} className="lg:w-4 lg:h-4 text-emerald-500" strokeWidth={3} />}
                        </div>
                      </button>
                    ))}
                  </div>
              ...
              <div className="space-y-1.5 lg:space-y-2">
                ...
                <div className="relative flex items-center">
                  <div className={`absolute left-3.5 sm:left-4 transition-colors ${formData.password ? 'text-emerald-500' : 'text-slate-400'}`}>
                    <Lock size={16} className="lg:w-5 lg:h-5" strokeWidth={2.4} />
                  </div>
                  ...
                  <div className="absolute right-3 sm:right-3.5 flex items-center gap-2">
                    {hasBiometrics && (
                      ...
                      >
                        {isBiometricLoading ? (
                          <Loader2 size={15} className="lg:w-5 lg:h-5 animate-spin text-emerald-500" />
                        ) : (
                          <Fingerprint size={16} className="lg:w-5 lg:h-5" strokeWidth={2.4} />
                        )}
                      </button>
                    )}
              ...
              >
                {isSubmitting ? (
                  <Loader2 size={16} className="lg:w-5 lg:h-5 animate-spin text-white" />
                ) : (
                  <>
                    <span>Sign In</span>
                    <ArrowRight size={16} className="lg:w-5 lg:h-5" strokeWidth={2.4} />
                  </>
                )}
              </button>
            </form>
