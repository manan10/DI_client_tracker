...
const mapHeadersToStandard = (headers) => {
  ...
  headers.forEach((h) => {
    ...
    Object.keys(HEADER_VARIANTS).forEach((standardKey) => {
      if (
        !mapping[standardKey] &&
        HEADER_VARIANTS[standardKey].some((variant) => clean.includes(variant))
      ) {
...
const parseRobust = (buffer) => {
  return new Promise((resolve, reject) => {
    ...
    Readable.from(cleanContent)
      .pipe(
        csv({ mapHeaders: ({ header }) => header.trim().replace(/\s+/g, " ") }),
      )
      .on("headers", (headers) => {
      ...
      .on("data", (row) => {
        ...
        const cleanNum = (val) =>
          val ? parseFloat(val.toString().replace(/,/g, "").trim()) || 0 : 0;
        ...
        results.push({
          ...
          narration:
            row[columnMap.narration]?.replace(/\s+/g, " ").trim() || "N/A",
...
const parseExcel = async (buffer, password = null) => {
  ...
  for (let i = headerIndex + 1; i < rows.length; i++) {
    ...
    const cleanNum = (val) =>
      val ? parseFloat(String(val).replace(/,/g, "").trim()) || 0 : 0;
    ...
    results.push({
      date: finalDateStr,
      narration: String(rowObj[columnMap.narration] || "N/A")
        .replace(/\s+/g, " ")
        .trim(),
...
const recalculateAuditSummary = async (auditId) => {
  ...
  audit.bankSummaries.forEach((bank) => {
    const bankTx = allTx.filter(
      (t) =>
        t.ledgerName === bank.tallyLedgerName ||
        t.bank === bank.tallyLedgerName,
    );

    const receipts = bankTx.filter((t) => t.type === "RECEIPT");
    const payments = bankTx.filter((t) => t.type === "PAYMENT");

    const sales = bankTx.filter(
      (t) =>
        t.type === "RECEIPT" &&
        (t.isSales === true || String(t.isSales).toLowerCase() === "true"),
    );
  ...
  audit.summary = {
    totalReceipts: audit.bankSummaries.reduce(
      (sum, b) => sum + (b.totalReceipts || 0),
      0,
    ),
    totalPayments: audit.bankSummaries.reduce(
      (sum, b) => sum + (b.totalPayments || 0),
      0,
    ),
    receiptCount: audit.bankSummaries.reduce(
      (sum, b) => sum + (b.receiptCount || 0),
      0,
    ),
    paymentCount: audit.bankSummaries.reduce(
      (sum, b) => sum + (b.paymentCount || 0),
      0,
    ),
    totalSales: audit.bankSummaries.reduce(
      (sum, b) => sum + (b.totalSales || 0),
      0,
    ),
    salesCount: audit.bankSummaries.reduce(
      (sum, b) => sum + (b.salesCount || 0),
      0,
    ),
  };
...
exports.getActiveAudit = async (req, res) => {
  try {
    ...
    const transactions = await Transaction.find({ auditId: audit._id }).sort({
      date: 1,
    });
...
exports.initializeAudit = async (req, res) => {
  try {
    const { accountId, tallyCompanyName, arnId, month, year, sourceFiles } =
      req.body;
  ...
  try {
    ...
    if (!arnDoc) {
      return res.status(404).json({
        success: false,
        message: `The company "${tallyCompany}" is not linked to any Client ARN.`,
      });
    }
    ...
    const ledgerMaster = await Ledger.find({
    ...
    for (const file of req.files) {
      ...
      try {
        ...
        if (parseError.message === "LOCKED_FILE") {
          return res.status(422).json({
            success: false,
            message: `The file "${file.originalname}" is password protected.`,
          });
        }
        throw parseError;
      }
      ...
      const enriched = parsedRows.map((t) => {
        ...
        if (t.type === "RECEIPT") {
          const narrationHit = commKeywords.some((k) =>
            cleanNarration.includes(k),
          );

          const ledgerHit =
            matchedLedgerName.includes("commission") ||
          ...
          if (isComm) {
            ...
            matchedLedgerName.includes("lic") ||
            matchedLedgerName.includes("insurance")
          ) {
            isSale = true;
          }
        }
    ...
    if (allTransactions.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "No valid transactions found." });
    }
    ...
    const sales = sortedTransactions.filter(
      (t) => t.type === "RECEIPT" && t.isSales,
    );
    ...
    const existingIndex = audit.bankSummaries.findIndex(
      (b) => b.tallyLedgerName === tallyLedger,
    );
    if (existingIndex >= 0) {
    ...
    console.log(
      `--- SUCCESS: ${saved.length} ROWS SAVED FOR ${tallyCompany} -> ${tallyLedger} ---`,
    );
...
exports.deleteAuditSession = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Audit session and transactions cleared",
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
  ...
  try {
    ...
    if (!transactions || !Array.isArray(transactions)) {
      return res.status(400).json({
        success: false,
        message: "Invalid data payload array required.",
      });
    }
    ...
    const bulkOperations = transactions.map((tx) => ({
      updateOne: {
        ...
        update: {
          $set: {
            isSalesApproved: Object.prototype.hasOwnProperty.call(
              tx,
              "isSalesApproved",
            )
              ? tx.isSalesApproved
    ...
    if (bulkOperations.length === 0)
      return res.json({
        success: true,
        message: "No staging row updates required",
      });
    ...
    return res.json({
      success: true,
      message: "Sales validation workbench state committed",
      matchedCount: result.matchedCount,
    });
  } catch (err) {
    ...
    return res.status(500).json({
      success: false,
      message: "Internal transactional exception handled",
    });
  }
};
...
exports.updateAuditSettings = async (req, res) => {
  try {
    ...
    if (!updatedAudit) {
      return res
        .status(404)
        .json({ success: false, message: "Audit session not found" });
    }
  ...
  try {
    ...
    if (!tallyCompany) {
      return res.status(400).json({
        success: false,
        message:
          "Tally Company Name is required to fetch the correct Ledger Master.",
      });
    }
    ...
    if (!ledgerMaster || ledgerMaster.length === 0) {
      return res.status(404).json({
        success: false,
        message: `No ledgers found for company: ${tallyCompany}`,
      });
    }
    ...
    try {
      ...
      if (parseError.message === "LOCKED_FILE") {
        return res
          .status(422)
          .json({ success: false, message: `The file is password protected.` });
      }
      throw parseError;
    }
